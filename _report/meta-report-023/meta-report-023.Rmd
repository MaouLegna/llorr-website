...
description: |
  Patch 2.15 - Week 1 - A new Rubin meta?
base_url: https://www.llorr-stats.com
preview: "https://dd.b.pvp.net/latest/set5/en_us/img/cards/05BW005-full.png"
author:
...
params:
  prev:  
  start: "2021-09-08 21:00:00" #UTC tz / 'current' week start
...
meta() %>%
  ...
  meta_social(
    ...
    image = "https://dd.b.pvp.net/latest/set5/en_us/img/cards/05BW005-full.png",
    image_alt = "Nami lv1",
    og_type = "website",
    og_author = "Legna",
...
In addition a quick fix to account for the Rubin-baits decks hase been applied and they are called:
...
nGames_tbl <- tbl_merge( 
  ...
  tab_header(
    ...
    subtitle = "Patch 2.15 first week / Master players"
...
# v1 <- LoR.Melt.Matches.RMD |>
#   filter(str_detect(player,"Draven / Sion")) |>
#   tabyl(player)
...
`r kableExtra::text_spec("Note :", color = "red")` While these are indeed the Match-Ups win-rates that one can extract from last week games I need to point out that this is not the most accurate estimation you can get from my data. If you want a better picture of the current meta it would be better to look at the MU-page where I use all games with the current sets of buffs and nerfs. While it's that there I don't account for optimization and difference in skills acquired during the weeks the overall number of games / sample size makes them a much safer source of information. While I'll continue to release the weekly data, pls refer to the [MU - page](https://www.llorr-stats.com/static/mu.html) for a better "meta-investigation".
...
```{r}
deck.of.week <- "RubinBait - Draven / Ezreal"

SubDeck_deck_code <- LoR.Melt.Matches.RMD |>
  # filter(str_detect(userID,"RubinZoo")) |>
  filter(player == "RubinBait - Draven / Ezreal") |>
  pull(deck_code)
...
```{r}
deck.of.week <- "RubinBait - Draven / Sion"
 
SubDeck_deck_code <- LoR.Melt.Matches.RMD |>
  filter(player == "RubinBait - Draven / Sion") |>
  pull(deck_code)
...
## Deckcodes
```{r}
BaitDeck <- LoR.Melt.Matches.RMD |>
  filter( str_detect(player,"Rubin" ) ) |>
  distinct(player) |> pull()
  ...
  reactable(.,
            ...
            columns = list(
              deck_code = colDef(name = "DeckCode", cell = function(value, index) {
                # Render as a link
                url <- sprintf("https://lor.runeterra.ar/decks/code/%s", value)
