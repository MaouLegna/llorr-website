...
LoR.Master.Matches.RMD <- LoR.Match.RMD %>%
  ...
  filter( game_start_time_cest >= as.POSIXct("2021-07-14 19:00:00", tz = "CEST") & game_start_time_cest < as.POSIXct("2021-07-21 19:00:00", tz = "CEST") ) %>%
...
```{r nGames-data}
live_patch_minrow   <- LoR.Match.RMD[,str_detect(game_version, '2_12') ] %>% which() %>% min()
...
Sample population: Ranked games / Patch 2.12 / Master players [^1]

Number of games: `r nGames %>% format(.,scientific = F)`
...
- Match Id recovered in patch 2.12: `r sum(nGames_live)` 
- Match metadata recovered in patch 2.12: `r nGames_live[which(names(nGames_live)=="200")]` 
...
- Match metadata still to collect from 2.12: `r nGames_live[which(is.na(names(nGames_live)))]` 
...
`r kableExtra::text_spec("Note:", color = "red")` While I'll use more the site some may prefer the format I use in RPubs with the toc so I'll try to also create an RPub version.
...
sxLabel <- ifelse(as.numeric(sxLabel) > 4.5,  sxLabel , "")
dxLabel <- ifelse(as.numeric(dxLabel) <= 4.5, dxLabel , "")
...
ggplot() +
  ...
  gghighlight::gghighlight(champions %in% fct_values[-(m+1)], label_key = champions, label_params = list(size = 2)) +
  ylim(NA,0.1) +
...
Win rates of the most played combination of champions, against all decks, this week. Play Rate >= 1%
...
Top Win rates of the least played combination of champions. Play rate $\in$ [0.1%,1%)[^3].
...
)) %>% mutate(userID = "Top10_Master") %>% select(userID,game_outcome)),
...
[^4]: [LMI - Early Theory](https://llorr-stats.netlify.app/analysis/lmi/)
