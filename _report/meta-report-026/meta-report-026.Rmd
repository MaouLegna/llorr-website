---
title: "THE META REPORT NAME IS TOO LONG, TOO DAMN LONG (n°26)"
description: |
  Patch 2.16 - Week 2 - Beyond the Bandlewood Seasonal Tournament waiting room
...
params:
  ...
  start: "2021-09-29 21:00:00" #UTC tz / 'current' week start
...
nGames_tbl <- tbl_merge( 
  ...
  tab_header(
    ...
    subtitle = "Patch 2.16 second week / Master playerDecks"
    #'**************************************************************************** *
...
ChampionCombo.plot <- CC.table |>
  ...
  labs(x = "Champions",
       ...
       caption = glue::glue("Ranked games from {params$start} UTC to {params$end} UTC.
                            Metadata of games collected with RiotGames API")
       ) +
  theme( 
    legend.position = "none",
...
meta_evo <- ggplot(
  ...
  labs(x = "Date",
       ...
       # caption = glue::glue("Data from Riot API • Ranked games from Master playerDecks • ... ")
  ) +
  theme_539() +
  theme( legend.position = "none",
...
setDT(LoR.Melt.Matches.RMD)[order(game_start_time_utc)][, counter := rowid(rleid(game_outcome)), by=c("playerID","playerDeck") ][ game_outcome == "win" ] |>
  ...
  select(server,gameName,counter,playerDeck,deck_code) |>
  mutate( server = str_to_title(server) ) |>
  group_by(server) |>
  ...
  reactable(
    ...
    columns = list(
      ...
      playerDeck = colDef(name = "Archetype",minWidth = 200),
...
metathis::meta() |>
  ...
  metathis::meta_social(
    ...
    image = "https://dd.b.pvp.net/latest/set5/en_us/img/cards/05BC133T1-full.png",
