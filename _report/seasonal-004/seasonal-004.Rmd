...
params:
  ind: "S2022-02-05"
  title: "THE SEASONAL REPORT IS VAST (n°4)"
...
deck_filter <- LoR.Match.RMD |>
  ...
  filter( game_start_time_utc >= (with_tz(force_tz(timeBlock.1.start,"Japan") + hours(1),"UTC")-lubridate::days(7)) & game_start_time_utc < (with_tz(force_tz(timeBlock.1.start,"Japan") + hours(1),"UTC")+days(2)) ) |>
  #' Base filters
...
LoR.Melt.Matches.RMD <- LoR.Match.RMD |>
  ...
  filter( game_start_time_utc %within% interval(
    with_tz(force_tz(timeBlock.1.start,"Japan") + hours(1),"UTC")-lubridate::days(7),
    with_tz(force_tz(timeBlock.1.start,"Japan") + hours(1),"UTC"))
  ) |>
  rename( playerPuuid_1 = puuid_1, playerPuuid_2 = puuid_2 ) |>
  mutate( opponentPuuid_1 = playerPuuid_2, opponentPuuid_2 = playerPuuid_1 ) |>
  mutate( opponent_deck_code_1 = deck_code_2, opponent_deck_code_2 = deck_code_1 ) |>
...
```{r check-value}
if (any(c(LoR.Melt.Seasonal[ is.na(playerID), .N ],
...
Game.Result.APAC <- LoR.Melt.Seasonal |>
  filter( server=="apac" ) |>
  ...
  mutate( round = case_when(
    ...
    # JST = UTC - 9hrs
    timeBlock > with_tz(force_tz(timeBlock.1.start,"Japan") + hours(1),"UTC") & timeBlock <= with_tz(force_tz(timeBlock.1.end,"Japan") + hours(1),"UTC") ~ 1, # 11:55
    timeBlock > with_tz(force_tz(timeBlock.2.start,"Japan") + hours(1),"UTC") & timeBlock <= with_tz(force_tz(timeBlock.2.end,"Japan") + hours(1),"UTC") ~ 2, # 13:00
    timeBlock > with_tz(force_tz(timeBlock.3.start,"Japan") + hours(1),"UTC") & timeBlock <= with_tz(force_tz(timeBlock.3.end,"Japan") + hours(1),"UTC") ~ 3, # 14:05
    timeBlock > with_tz(force_tz(timeBlock.4.start,"Japan") + hours(1),"UTC") & timeBlock <= with_tz(force_tz(timeBlock.4.end,"Japan") + hours(1),"UTC") ~ 4, # 15:10
    timeBlock > with_tz(force_tz(timeBlock.5.start,"Japan") + hours(1),"UTC") & timeBlock <= with_tz(force_tz(timeBlock.5.end,"Japan") + hours(1),"UTC") ~ 5, # 16:15
    
    timeBlock > with_tz(force_tz(timeBlock.6.start,"Japan") + hours(1),"UTC") & timeBlock <= with_tz(force_tz(timeBlock.6.end,"Japan") + hours(1),"UTC") ~ 6, # 17:45
    timeBlock > with_tz(force_tz(timeBlock.7.start,"Japan") + hours(1),"UTC") & timeBlock <= with_tz(force_tz(timeBlock.7.end,"Japan") + hours(1),"UTC") ~ 7, # 18:50
    timeBlock > with_tz(force_tz(timeBlock.8.start,"Japan") + hours(1),"UTC") & timeBlock <= with_tz(force_tz(timeBlock.8.end,"Japan") + hours(1),"UTC") ~ 8, # 19:55
    timeBlock > with_tz(force_tz(timeBlock.9.start,"Japan") + hours(1),"UTC") ~ 9, # 21:00
    )
  ) |>
  ...
  {\(x)
  left_join(
  tibble( playerID = rep( LineUp.DT |>filter(server=="apac") |>pull(playerID),each=9),
...
# filter(str_detect(playerID,"Dudu") )
# LoR.Melt.Seasonal[ is.na(playerID) ]
```
...
#   group_by(  playerID,opponentID,server,timeBlock ) |>
#   arrange( timeBlock ) |>
#   filter( timeBlock > as.POSIXct("2021-11-27 19:00:00",tz = "UTC") ) |>
#   summarise( t = min(timeBlock) ) |>
#   arrange(playerID,timeBlock)
```
...
Game.Result.NA <- LoR.Melt.Seasonal |>
  ...
  mutate( round = case_when(
    ...
    timeBlock > with_tz(force_tz(timeBlock.2.start,"US/Central"),"UTC")+minutes(25) & timeBlock <= with_tz(force_tz(timeBlock.2.end,"US/Central"),"UTC")+minutes(25) ~ 2, # 13:00
    timeBlock > with_tz(force_tz(timeBlock.3.start,"US/Central"),"UTC")+minutes(25) & timeBlock <= with_tz(force_tz(timeBlock.3.end,"US/Central"),"UTC")+minutes(25) ~ 3, # 14:05
    timeBlock > with_tz(force_tz(timeBlock.4.start,"US/Central"),"UTC")+minutes(25) & timeBlock <= with_tz(force_tz(timeBlock.4.end,"US/Central"),"UTC")+minutes(25) ~ 4, # 15:10
    timeBlock > with_tz(force_tz(timeBlock.5.start,"US/Central"),"UTC")+minutes(25) & timeBlock <= with_tz(force_tz(timeBlock.5.end,"US/Central"),"UTC")+minutes(25) ~ 5, # 16:15
    
    timeBlock > with_tz(force_tz(timeBlock.6.start,"US/Central"),"UTC")+minutes(25) & timeBlock <= with_tz(force_tz(timeBlock.6.end,"US/Central"),"UTC")+minutes(25) ~ 6, # 17:45
    timeBlock > with_tz(force_tz(timeBlock.7.start,"US/Central"),"UTC")+minutes(25) & timeBlock <= with_tz(force_tz(timeBlock.7.end,"US/Central"),"UTC")+minutes(25) ~ 7, # 18:50
    timeBlock > with_tz(force_tz(timeBlock.8.start,"US/Central"),"UTC")+minutes(25) & timeBlock <= with_tz(force_tz(timeBlock.8.end,"US/Central"),"UTC")+minutes(25) ~ 8, # 19:55
    timeBlock > with_tz(force_tz(timeBlock.9.start,"US/Central"),"UTC")+minutes(25) ~ 9, # 21:00
    )
  ) |>
  # number of won games in the match
  mutate( nWin = str_count(result,"win") ) |>
...
ggsave(glue::glue("images/{params$ind}-RegionPR.png"),plot = Region.Freq.plot, width = 12, height = 8)
...
<!--   group_by(server) |> -->
<!--   filter( (server == "apac" & cumMatchWin >= 6) | (server != "apac" & cumMatchWin >= 7) ) |> -->
...
<!-- ggsave(glue::glue("./images/seasonal/{params$ind}-champ-top-Apac.png"),width = 12,height = 8) -->
...
<!--     inset_element(ggtop32.APAC.pie, left = 0.5, right = 1, bottom = 0, top = 0.8), -->
<!--   ggplot() + -->
<!--     theme_539() + -->
<!--     annotation_custom(grid::rasterGrob(ft.top32tbl.CC.APAC), xmin=-Inf, xmax=Inf, ymin=-Inf, ymax=Inf), nrow = 1, ncol = 2, rel_widths = c(4, 1) ) -->

<!-- ggsave(glue::glue("./images/seasonal/{params$ind}-top-Apac.png"),plot = APAC.finalplot,width = 14,height = 8) -->
<!-- ``` -->
<!-- ::: -->
...
<!--   ggimage::geom_image(aes(x = name, image = img), y = 0, size = 0.04,inherit.aes = FALSE) + -->
<!--   coord_flip() + -->
<!--   scale_fill_manual("legend", values = set_names(data_regions$colorRegion, data_regions$regionRef  ) ) + -->
<!--   theme_539() + -->
<!--   labs( x = "Champion", -->
...
<!-- ggsave(glue::glue("./images/seasonal/{params$ind}-champ-top-EU.png"),width = 12,height = 8) -->
...
<!--   ggtop32.EU + -->
<!--     theme(panel.grid.major.x = element_blank()) +  -->
<!--     inset_element(ggtop32.EU.pie, left = 0.5, right = 1, bottom = 0, top = 0.8), -->
<!--   ggplot() + -->
<!--     theme_539() + -->
<!--     annotation_custom(grid::rasterGrob(ft.top32tbl.CC.EU), xmin=-Inf, xmax=Inf, ymin=-Inf, ymax=Inf), nrow = 1, ncol = 2, rel_widths = c(4, 1) ) -->

<!-- ggsave(glue::glue("./images/seasonal/{params$ind}-top-EU.png"),plot = EU.finalplot,width = 14,height = 8) -->
<!-- ``` -->
<!-- ::: -->
...
<!-- ggsave(glue::glue("./images/seasonal/{params$ind}-top-NA.png"),plot = NA.finalplot,width = 14,height = 8) -->
<!-- ``` -->
<!-- ::: -->
...
CC.table.S |>
  ...
  labs(
    ...
    caption = element_text(
      glue::glue("Playrates from {(with_tz(force_tz(timeBlock.1.start,'Japan'),'UTC')-lubridate::days(7))} to {with_tz(force_tz(timeBlock.1.start,'Japan'),'UTC')}, one week before the start of the earliest Seasonal start (the Apacn shard)
...
ggsave("images/{params$ind}-PRcor.png")
```
