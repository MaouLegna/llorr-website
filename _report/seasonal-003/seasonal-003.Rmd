...
params:
  ...
  title: "THE SEASONAL REPORT IS VAST (n°3)"
  description: "Seasonal Tournament - Between World - Open Rounds"
  cardlurl: "https://dd.b.pvp.net/latest/set1/en_us/img/cards/01DE001-full.png"
  seasonalDate: "2021-11-27"
...
nSeasonal = LoR.Melt.Seasonal |>
  mutate( server = factor(server, levels = c("asia","europe","americas")) ) |>
...
Game.Result.ASIA <- LoR.Melt.Seasonal |>
  ...
  mutate( round = case_when(
    ...
    timeBlock > with_tz(force_tz(timeBlock.2.start,"Japan"),"UTC")+minutes(145) & timeBlock <= with_tz(force_tz(timeBlock.2.end,"Japan"),"UTC")+minutes(145) ~ 2, # 13:00
    timeBlock > with_tz(force_tz(timeBlock.3.start,"Japan"),"UTC")+minutes(145) & timeBlock <= with_tz(force_tz(timeBlock.3.end,"Japan"),"UTC")+minutes(145) ~ 3, # 14:05
    timeBlock > with_tz(force_tz(timeBlock.4.start,"Japan"),"UTC")+minutes(145) & timeBlock <= with_tz(force_tz(timeBlock.4.end,"Japan"),"UTC")+minutes(145) ~ 4, # 15:10
    timeBlock > with_tz(force_tz(timeBlock.5.start,"Japan"),"UTC")+minutes(145) & timeBlock <= with_tz(force_tz(timeBlock.5.end,"Japan"),"UTC")+minutes(145) ~ 5, # 16:15
    
    timeBlock > with_tz(force_tz(timeBlock.6.start,"Japan"),"UTC")+minutes(145) & timeBlock <= with_tz(force_tz(timeBlock.6.end,"Japan"),"UTC")+minutes(145) ~ 6, # 17:45
    timeBlock > with_tz(force_tz(timeBlock.7.start,"Japan"),"UTC")+minutes(145) & timeBlock <= with_tz(force_tz(timeBlock.7.end,"Japan"),"UTC")+minutes(145) ~ 7, # 18:50
    timeBlock > with_tz(force_tz(timeBlock.8.start,"Japan"),"UTC")+minutes(145) & timeBlock <= with_tz(force_tz(timeBlock.8.end,"Japan"),"UTC")+minutes(145) ~ 8, # 19:55
    timeBlock > with_tz(force_tz(timeBlock.9.start,"Japan"),"UTC")+minutes(145) ~ 9, # 21:00
    )
  ) |>
...
gtsave(gtLMI,glue::glue("./images/seasonal/{params$ind}-LMI-table-v2.png") ) |> invisible()
```
...
ggsave(glue::glue("./images/seasonal/{params$ind}-RegionPR.png"),plot = Region.Freq.plot, width = 12, height = 8)
...
Region.Freq.tbl <- Region.Freq |>
  # mutate( server = factor(server, levels = c("asia","europe","americas")) ) |>
  select( region,freq,contains("asia"),contains("europe"),contains("americas")) |>
  # rename_all(str_to_title) |>
  arrange(desc(freq)) |>
  gt() |>
  tab_header(
  ...
  tab_spanner(
    label = "Shard",
    columns = c(contains("asia"),contains("europe"),contains("americas"))
  ) |>
  fmt_percent(
    columns = -1,
    decimals = 2
...
- For the *all Shards - Asia,Europe and America* it displays those who reached at least 7 matches wins (not counting bye)
...
top32tbl <- Game.Result.all |>
  ...
  group_by(server) |>
  filter( (server == "asia" & cumMatchWin >= 6) | (server != "asia" & cumMatchWin >= 7) ) |>
...
```{r create-top32-ASIA, fig.height=8, fig.width=14}
...
ggsave(glue::glue("./images/seasonal/{params$ind}-champ-top-Asia.png"),width = 12,height = 8)
...
# Make the plot
ggtop32.ASIA.pie <- ggplot(data.region.ASIA, aes(ymax=ymax, ymin=ymin, xmax=4, xmin=3, fill=factions)) +
     geom_rect() +
...
#' unite the plot and table
ASIA.finalplot <- cowplot::plot_grid(
  ggtop32.ASIA + 
    theme(panel.grid.major.x = element_blank()) + 
    inset_element(ggtop32.ASIA.pie, left = 0.5, right = 1, bottom = 0, top = 0.8),
  ggplot() +
    theme_539() +
    annotation_custom(grid::rasterGrob(ft.top32tbl.CC.ASIA), xmin=-Inf, xmax=Inf, ymin=-Inf, ymax=Inf), nrow = 1, ncol = 2, rel_widths = c(4, 1) )

ggsave(glue::glue("./images/seasonal/{params$ind}-top-Asia.png"),plot = ASIA.finalplot,width = 14,height = 8)
```
:::
