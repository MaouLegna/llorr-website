...
#   arrange( timeBlock ) |>
#   filter( timeBlock > as.POSIXct("2021-11-27 04:00:00",tz = "UTC") ) |>
...
Game.Result.NA <- LoR.Melt.Seasonal |>
  ...
  # Identify the round with the official schedule
  mutate( round = case_when(
    timeBlock > with_tz(force_tz(timeBlock.1.start,"US/Central"),"UTC")-hours(2) & timeBlock <= with_tz(force_tz(timeBlock.1.end,"US/Central"),"UTC")-hours(2) ~ 1, # 11:55
    timeBlock > with_tz(force_tz(timeBlock.2.start,"US/Central"),"UTC")-hours(2) & timeBlock <= with_tz(force_tz(timeBlock.2.end,"US/Central"),"UTC")-hours(2) ~ 2, # 13:00
    timeBlock > with_tz(force_tz(timeBlock.3.start,"US/Central"),"UTC")-hours(2) & timeBlock <= with_tz(force_tz(timeBlock.3.end,"US/Central"),"UTC")-hours(2) ~ 3, # 14:05
    timeBlock > with_tz(force_tz(timeBlock.4.start,"US/Central"),"UTC")-hours(2) & timeBlock <= with_tz(force_tz(timeBlock.4.end,"US/Central"),"UTC")-hours(2) ~ 4, # 15:10
    timeBlock > with_tz(force_tz(timeBlock.5.start,"US/Central"),"UTC")-hours(2) & timeBlock <= with_tz(force_tz(timeBlock.5.end,"US/Central"),"UTC")-hours(2) ~ 5, # 16:15
    
    timeBlock > with_tz(force_tz(timeBlock.6.start,"US/Central"),"UTC")-hours(2) & timeBlock <= with_tz(force_tz(timeBlock.6.end,"US/Central"),"UTC")-hours(2) ~ 6, # 17:45
    timeBlock > (with_tz(force_tz(timeBlock.7.start,"US/Central"),"UTC")-hours(2)) & (timeBlock <= with_tz(force_tz(timeBlock.7.end,"US/Central"),"UTC")-hours(2)) ~ 7, # 18:50
    timeBlock > (with_tz(force_tz(timeBlock.8.start,"US/Central"),"UTC")-hours(2)) & (timeBlock <= with_tz(force_tz(timeBlock.8.end,"US/Central"),"UTC")-hours(2)) ~ 8, # 19:55
    timeBlock > (with_tz(force_tz(timeBlock.9.start,"US/Central"),"UTC")-hours(2)) ~ 9, # 21:00
    )
  ) |>
  # number of won games in the match
  mutate( nWin = str_count(result,"win") ) |>
...
ggtop32.ASIA <- top32tbl.CC  |>
  ...
  ggimage::geom_image(aes(x = name, image = img), y = 0, size = 0.1,inherit.aes = FALSE) +
  coord_flip() +
