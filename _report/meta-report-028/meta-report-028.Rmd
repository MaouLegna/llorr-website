...
LoR.Melt.Seasonal <- LoR.Match.RMD |>
  ...
  filter( game_start_time_utc >= as.POSIXct(params$end,"UTC")-days(5) & game_start_time_utc < as.POSIXct(params$end,"UTC") ) |>
  select( -ends_with("_3"),-ends_with("_4")  ) |>
  left_join(LoR.Deck[,.(deck_code,playerDeck_1=archetype)],  by=c("deck_code_1"="deck_code")) |>
  left_join(LoR.Deck[,.(deck_code,opponentDeck_1=archetype)],by=c("deck_code_2"="deck_code")) |>
  rename( playerPuuid_1 = puuid_1, playerPuuid_2 = puuid_2 ) |>
...
LoR.Melt.Seasonal |>
  ...
  count(game_outcome) |>
  ungroup() |>
  filter(game_outcome=="win") |>
  # arrange(desc(n)) |>
  rename(sWin=n) |>
  select(-game_outcome) |>
  left_join(SLMI.DT, by = c("playerDeck"="deck")) |>
  select(playerDeck,LMI,WR,sWin) |>
  summarize(corLMI = cor(LMI, sWin, method = "sp", use = "complete.obs" ),
            corWR  = cor(WR, sWin, method = "sp", use = "complete.obs" ))
...
LoR.Melt.Seasonal |>
  ...
  filter(game_outcome=="win") |>
  # arrange(desc(n)) |>
  rename(sWin=n,deck=playerDeck) |>
  select(-game_outcome) |>
  left_join(LMI.DT, by = "deck") |>
  mutate( 
    rankTier = factor(tier, levels = c("Tier3","Tier2","Tier1","Tier0"),ordered = T ),
    rankTier = as.numeric(rankTier)
    ) |>
  select(deck,sWin,LMI,rankTier,nWin,nGames,WR,nBan,maxBan,meanBan,n,playrate,lWR) |>
  # relocate( sWin,.before = "LMI" ) |>
  ...
  text_transform(
    ...
    fn = function(x){
      ...
      n <- word(x, -1)
      glue::glue(
        "<div style='line-height:10px'><span style='font-weight:bold;font-variant:small-caps;font-size:14px'>{n}</div>
        <div style='line-height:12px'><span style ='font-weight:bold;font-size:10px'>({percent})</span></div>"
      )
    }
  ) |>
  ...
  gtExtras::gt_color_rows(sWin:LMI,reverse = T) |>
  cols_label(
    rankTier = md("Tier"),
    WR   = md("WinRate"),
    meanBan  = md("BanRate"),
  ...
  tab_source_note(
    source_note = md(glue::glue("Decks with at least 50 games being played\n
...
# gtsave(gtLMI,glue::glue("./images/seasonal/S2021-10-09-LMI-table-v2.png") ) |> invisible()
```
