...
params:
  ...
  cardlurl: "https://dd.b.pvp.net/latest/set5/en_us/img/cards/05SH014T1-full.png"
...
```{r}
# LoR.Match.RMD[ ,.N  ]
# Temp.Match.RMD[match_key %in% LoR.Match.RMD$match_key, .N ]
# LoR.Match.RMD[ match_key %!in% Temp.Match.RMD$match_key  ]
```
...
# LoR.Melt.Matches.RMD[ playerID == "Agigas#EUW" ] |>
#   filter(playerDeck == "Miss Fortune/Poppy (BW/DE)") |>
#   tabyl(game_outcome)
```
...
Number of (Ranked) matches analysed **`r format(nMatches,scientific = F)`** or **`r format(nGames,scientific = F)`** games.
...
```{r}
# For GIF
animate(anim, 50, fps = 25,  width = 1200, height = 640)
# animate(anim, 50, fps = 20,  width = 600, height = 500, 
#         renderer = gifski_renderer("images/meta-report/{params$ind}-PRanimate.gif"))
...
#   filter( playerDeck == "Braum/Poppy (BC/FR)") |>
#   tabyl(playerID) |>
#   arrange(desc(n))
...
gtWinning <- setDT(LoR.Melt.Matches.RMD)[order(game_start_time_utc)][, counter := rowid(rleid(game_outcome)), by=c("playerID","playerDeck") ][ game_outcome == "win" ] |>
  ...
  tab_options(
    ...
    column_labels.border.top.color = "whitesmoke",
    table.border.top.color = "whitesmoke",
    table.border.bottom.color = "whitesmoke",
