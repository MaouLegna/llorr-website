...
params:
  ind: "MR071"
  patch: "Patch 3.12 - Week 2"
  title: "THE META REPORT NAME IS TOO LONG, TOO DAMN LONG (n°71)"
  description: "First Patch of Ferragosto, Refreshes the Meta (Actually true?!)"
  cardlurl: "https://dd.b.pvp.net/latest/set6/en_us/img/cards/06RU001T4-full.png"
  start: "2022-08-10 18:00:00" #UTC tz / 'current' week start
  end:   "2022-08-17 18:00:00" #UTC tz / 'current' week end
  nGrid: 10
  nDeck_cc:  20
  nDeck_day: 12
  min_games: 50
...
# load Games DT
LoR_Match_DT_RMD <- list.files(path = file.path("C:", "LlorR", "data", "raw"), pattern = glue("^(.*)Match_DT_S14(.*)csv$"), full.names = T) |>
  map_dfr(data.table::fread, header = T, na.strings = c("", NA), encoding = "UTF-8") |>
  mutate(server = if_else(server %in% c("sea", "asia"), "apac", server) )

LoR_Diamond_DT_RMD <- list.files(path = file.path("C:", "LlorR", "data", "raw"), pattern = glue("^(.*)Diamond_DT_S14(.*)csv$"), full.names = T) |>
  map_dfr(data.table::fread, header = T, na.strings = c("", NA), encoding = "UTF-8") |>
  mutate(server = if_else(server %in% c("sea", "asia"), "apac", server) )
