...
params:
  ...
  title: "THE META REPORT NAME IS TOO LONG, TOO DAMN LONG (n°36)"
  description: "Gathering a Stom of new Tier1 decks."
...
#' Read Diamond
###############
LoR.Diamond.DT.RMD   <- fread(file.path("C:","LlorR","data","raw","LoR_DiamondDT.csv"), header = T, na.strings = c("", NA))

#' Read Temp
############
LoR.Old.DT.RMD   <- fread(file.path("C:","LlorR","data","raw","old-data","LoR_MatchDT_S10_218_220.csv"), header = T, na.strings = c("", NA))
...
```{r account-info}
masterEU <- fread(file.path("C:","LlorR","data","raw","master","LoR_Master_S11_EU.csv"),
                              header = T, na.strings = c("", NA), encoding = "UTF-8") |>
  filter(time <= as.POSIXct(params$end,"UTC"))

masterNA <- fread(file.path("C:","LlorR","data","raw","master","LoR_Master_S11_NA.csv"),
                              header = T, na.strings = c("", NA), encoding = "UTF-8") |>
  filter(time <= as.POSIXct(params$end,"UTC"))

masterASIA <- fread(file.path("C:","LlorR","data","raw","master","LoR_Master_S11_AS.csv"),
                              header = T, na.strings = c("", NA), encoding = "UTF-8") |>
  filter(time <= as.POSIXct(params$end,"UTC"))
...
Region.Freq.tbl <- Region.Freq |>
  relocate(nameRef,.after="n") |>
  select(nameRef:europe) |>
  arrange(desc(freq)) |>
  gt( ) |>
  tab_header(
...
::: {.panel}
### from Last-Seasononal {.panel-name}
...
```{r}
grid <- matrix(MUtblTop.Master$muWR, nrow = mu, ncol = mu, byrow = T)
diag(grid) <- NA
colnames(grid) <- TopCC

grid.diamond <- matrix(MUtblTop.Diamond$muWR, nrow = mu, ncol = mu, byrow = T)
...
whichDecks <- LoR.Melt.Diamond.RMD |>
