...
params:
  ind: "MR034"
  patch: "Patch 2.20 - Week 1"
  title: "THE META REPORT NAME IS TOO LONG, TOO DAMN LONG (n°34)"
  description: "Arcane may be over (for now) but the Seasonal is not"
  cardlurl: "https://dd.b.pvp.net/latest/set1/en_us/img/cards/01PZ040T1-full.png"
  prev:  "2021-11-17 18:00:00" #UTC tz / 'current' previous week start
  start: "2021-11-24 18:00:00" #UTC tz / 'current' week start
  end:   "2021-12-01 18:00:00" #UTC tz / 'current' week end
  nGrid: 15
...
Nothing "new" this week, but wanted to remind the existence of the deck. Discard, after all made the community realize how 3 or 4 damage (to face) are big way before Arcane.
...
## Draven/Jinx a.k.a Discard (aggro)

```{r deckweek, layout="l-page"}
deck.of.week <- "Draven/Jinx"
...
Card.Subset <- LoR.Melt.Master.RMD |>
  select( starts_with( "Card" ) )

allCards            <- unlist(apply(Card.Subset, 1, unique),use.names = F)
TableAllCards       <- tabyl(allCards) |>
  mutate( abbreviation = str_sub(allCards,3,4) ) |>
  rename( card = allCards, N = n ) |>
  as.data.table() |>
  left_join(Region.Freq[,c("region","n")]|>left_join(data_regions[,c("nameRef","abbreviation")], by=c("region"="nameRef") ), by = c("abbreviation") ) |>
  mutate( p.byReg = round(N/n,3),
  ...
  select(card,N,region,nGamesReg=n,p.byReg)
```
