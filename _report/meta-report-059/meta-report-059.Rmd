...
params:
  ...
  description: "Jhin Jhin Jhin Jhin"
  cardlurl: ""
...
```
```{r archetype-fix}
LoR.Deck.RMD <- archetype_pretty(LoR.Deck.RMD)
...
LoR.Melt.Master.RMD <- LoR.Match.DT.RMD |>
  ...
  melt_match(LoR.Deck.RMD,LoR.Account.RMD)
...
```{r account-info}
EU.master.found   <- LoR.Account.RMD[ activeShard == "europe" & master == "master", .N ]
NA.master.found   <- LoR.Account.RMD[ activeShard == "americas" & master == "master", .N ]
APAC.master.found <- LoR.Account.RMD[ activeShard == "apac" & master == "master", .N ]

EU.master.ladder   <- lorR::lor_leaderboard("europe",names=T) |> length()
NA.master.ladder   <- lorR::lor_leaderboard("americas",names=T) |> length()
APAC.master.ladder <- lorR::lor_leaderboard("sea",names=T) |> length() 
```
...
LoR.Seasonal.DT.RMD <- list.files(path = file.path("C:", "LlorR", "data", "raw"), pattern = glue("^(.*)Seasonal(.*)csv$"), full.names = T) |>
  ...
  filter(game_start_time_utc >= as_datetime("2022-05-20") )
...
# prepare
LoR.Deck.Seasonal <- LoR.Seasonal.DT.RMD |>
  extract_deck_code_info() |>
  archetype_pretty()

LoR.Melt.Seasonal <- LoR.Seasonal.DT.RMD |>
  melt_match(LoR.Deck.Seasonal,LoR.Account.RMD)
...
# results
Game.Results <- LoR.Melt.Seasonal |>
...
gt_winlosstbl <- gt_winlosstbl |>
  ...
  tab_header(
    ...
    subtitle = md("Game results from the top32 players of the 'A Curious Journey' Seasonal Tournament") 
  ) |>
  tab_source_note(source_note = md("Metadata of games collected with RiotGames API")) |>
  gtExtras::gt_theme_538()

gtsave(gt_winlosstbl,glue::glue("curious-journey-playoff-winloss.png"))
...
gt_winlosstbl <- winlosstbl |>
  select(WonMatches,sort(current_vars())) |>
  gt() |>
  tab_spanner_delim(delim = "_") |>
  fmt_missing(
    columns=everything(),
...
#   reactablefmtr::add_source(glue::glue("Source: Metadata of games collected with RiotGames API - by Legna - https://www.llorr-stats.com"), background_color = "white")
# 
