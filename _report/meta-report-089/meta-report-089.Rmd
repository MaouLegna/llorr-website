...
params:
  ind: "MR089"
  season: "S17"
  patch: "Patch 3.21 - Week 2"
  title: "THE META REPORT NAME IS TOO LONG, TOO DAMN LONG (n°89)"
  description: "Santa Kayle bringing cinder to all childrens. Well, turning sinner (all) children to cinder."
  cardlurl: "http://dd.b.pvp.net/3_21_0/set6cde/en_us/img/cards/06MT008T2-full.png"
  start: "2022-12-14 18:00:00" #UTC tz / 'current' week start
  end:   "2022-12-21 18:00:00" #UTC tz / 'current' week end
  nGrid: 10
  nDeck_cc:  20
  nDeck_day: 12
  min_games: 10
...
```{r update-dataset}
update_lor_database(server_abbr = "EU",last_season = params$season,delete = TRUE)
update_lor_database(server_abbr = "NA",last_season = params$season,delete = TRUE)
update_lor_database(server_abbr = "APAC",last_season = params$season,delete = TRUE)
```
...
```{r load-data}
# load DeckDT
LoR_Deck_RMD       <- data.table::fread(file.path("C:", "LlorR", "data", "raw", glue::glue("LoR_DECK_{params$season}.csv")), na.strings = c("", NA), colClasses = "character")

# load Games DT
LoR_Match_DT_RMD <- list.files(path = file.path("C:", "LlorR", "data", "raw"), pattern = glue("^(.*)Match_DT_{params$season}(.*)csv$"), full.names = T) |>
...
LoR_Diamond_DT_RMD <- list.files(path = file.path("C:", "LlorR", "data", "raw"), pattern = glue("^(.*)Diamond_DT_{params$season}(.*)csv$"), full.names = T) |>
  map_dfr(data.table::fread, header = T, na.strings = c("", NA), encoding = "UTF-8") |>
  mutate(server = if_else(server %in% c("sea", "asia"), "apac", server) ) |>
  filter(game_start_time_utc %within% interval(as.POSIXct(params$start, "UTC"), as.POSIXct(params$end, "UTC")) )
...
LoR_Melt_Master_RMD[ !is.na(cards_fix), cards:=cards_fix  ]
