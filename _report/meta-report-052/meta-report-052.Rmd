...
params:
  ind: "MR052"
  patch: "Patch 3.4 - Week 1"
  title: "THE META REPORT NAME IS TOO LONG, TOO DAMN LONG (n°52)"
  description: "Tell the people the Meta you have seen this week! (ALSO ONE YEAR!)"
  cardlurl: "https://dd.b.pvp.net/latest/set4/en_us/img/cards/04SH003-full.png"
  start: "2022-03-30 18:00:00" #UTC tz / 'current' week start
  end:   "2022-04-06 18:00:00" #UTC tz / 'current' week end
  nGrid: 10
  nDeck.CC:  20
  nDeck.Day: 12
  min.games: 5
...
master.NA <- fread(file.path("C:","LlorR","data","raw","account","LoR_Master_S12_NA.csv"), header = T, na.strings = c("", NA), encoding = "UTF-8",colClasses = "character")

master.NA <- update_current_master(DT = master.NA,DTA = LoR.Account.RMD,names = lor_leaderboard("americas",names = T),job.server.filter = "americas")

master.NA[ gameName =="HKGCrisper07", time:=as.character("2202-02-24 04:30:00") ]
master.NA[ gameName =="IanF",         time:=as.character("2022-04-04 22:30:00") ]
master.NA[ gameName =="HKGCrisper07", time:=as.character("2022-04-04 18:00:00") ]
master.NA[ gameName =="HKGCrisper07", time:=as.character("2022-04-05 18:00:00") ]

master.NA <- master.NA |>
  filter(time <= as.POSIXct(params$end,"UTC"))

master.APAC <- fread(file.path("C:","LlorR","data","raw","account","LoR_Master_S12_APAC.csv"), header = T, na.strings = c("", NA), encoding = "UTF-8",colClasses = "character")
  
master.APAC <- update_current_master(DT = master.APAC,DTA = LoR.Account.RMD,names = lor_leaderboard("sea",names = T),job.server.filter = "sea")
