...
params:
  ind: "MR035"
  patch: "Patch 2.20 - Week 2"
  title: "THE META REPORT NAME IS TOO LONG, TOO DAMN LONG (n°35)"
  description: "The light at the end of Beyond the Bandlewood tunnel"
  cardlurl: "https://dd.b.pvp.net/latest/set5/en_us/img/cards/05BC069-full.png"
  prev:  "2021-11-24 18:00:00" #UTC tz / 'current' previous week start
  start: "2021-12-01 18:00:00" #UTC tz / 'current' week start
  end:   "2021-12-08 18:00:00" #UTC tz / 'current' week end
  nGrid: 15
...
# mutate( allegiance = na_if(allegiance,"") )
```
...
LoR.Melt.Seasonal.RMD <- LoR.Match.RMD |>
...
```{r check-value}
if (any(c(LoR.Melt.Matches.RMD[ is.na(playerID), .N ],
          LoR.Melt.Master.RMD[  is.na(playerID), .N ],
...
```{r create-lineUp-DT}
LineUp.DT <- LoR.Melt.Seasonal.RMD |>
...
```{r create-game-result}
# LoR.Melt.Seasonal.RMD |>
#   group_by(server) |>
#   distinct(playergameName) |>
#   summarise(nplayer = n())

Game.Results <- LoR.Melt.Seasonal.RMD |>
  ...
  arrange( server,playerID,timeBlock ) |>
...
top32tbl <- Game.Results |>
  group_by(playergameName) |>
  slice_max(cumMatchWin,n=1,with_ties = F) |>
  ungroup() |>
  distinct(playergameName,.keep_all = T) |>
  select(server,playergameName,cumMatchWin,LU,contains("deck_"),contains("code_") ) |>
...
`r kableExtra::text_spec("Note:", color = "red")` As not all decks has being played I can't provide a complete coverage of the lineUps but `r glue("{deckFound}/{32*3*3}")` decks overall.
...
```{r check-seasonal-32}
winlosstbl <- Game.Results |>
  group_by(server,playergameName) |>
  ...
  left_join(
    Game.Results |>
      select(playergameName,cumMatchWin) |>
      group_by(playergameName) |>
      slice_max(cumMatchWin,n=1,with_ties=F),
    by = "playergameName"
  ) |>
  ...
  rename(WonMatches=cumMatchWin,Player=playergameName) |>
  pivot_wider(names_from = "server",    values_from=c(Player,result), names_glue = "{server}_{.value}" ) |>
  select(-row) |>
  gt() |>
  tab_spanner_delim(
      delim="_"
  ...
  gtExtras::gt_plt_winloss(contains("result"), max_wins = 20) |>
  # tab_options(data_row.padding = px(2)) |>
  tab_header(
    title = md("Top32 Results"),
    subtitle = md("Game results from the top32 players of the Between Worlds Seasonal Tournament") 
  ) |>
  tab_source_note(source_note = md("Metadata of games collected with RiotGames API")) |>
  gtExtras::gt_theme_538()


gtsave(winlosstbl,glue::glue("./images/meta-report/{params$ind}-winloss.png")) |> invisible()
...
react.LU.top32 <- top32tbl |>
  ...
  reactable(
    ...
    groupBy = c("server","cumMatchWin","playergameName"),
    bordered = TRUE,
    highlight = TRUE,
    searchable = TRUE,
    ...
    columns = list(
      ...
      playergameName = colDef(name = "Player" ),
      # code = colDef( show = F ),
  ...
  reactablefmtr::add_title("Top32 Players' Deckcodes", background_color = "whitesmoke", font_family = "Helvetica") |>
  reactablefmtr::add_subtitle(c("Bewtween Worlds Playoff - Top32 LineUps"), background_color = "whitesmoke", font_family = "Roboto") |>
  reactablefmtr::add_source(glue::glue("As not all decks has being played I can't provide a complete coverage of the lineUps but {deckFound}/{32*3*3} decks overall.
                                       Source: Metadata of games collected with RiotGames API"), background_color = "whitesmoke", font_family = "Chivo")

react.LU.top32 |>
  reactablefmtr::save_reactable(glue::glue("./images/seasonal/{params$ind}-top32-final.png"))

react.LU.top32 |>
  reactablefmtr::save_reactable(glue::glue("./images/meta-report/{params$ind}-top32-final.png"))
...
`r kableExtra::text_spec("Note:", color = "green")` the dual region factor has been solved
...
Regions.Freq.byCards <- LoR.Melt.Master.RMD |>
  ...
  separate_rows( cards.region,cards.region.freq, sep = "," ) |>
  mutate( n = as.numeric(cards.region.freq) ) |>
  ...
  left_join(data_regions[,c("abbreviation","iconsPath")] , by = c("region"="abbreviation") ) |>
  rename( icon = iconsPath )
...
Regions.Freq.byCards.Server <- LoR.Melt.Master.RMD |>
  ...
  mutate( freq = n / sum(n) ) |>
  ungroup() |>
  rename_all(~c("region","server","n","freq")) |>
  arrange(region) |>
  mutate( freq = round(freq,4) )
...
Region.Freq.byCards.tbl <- Regions.Freq.byCards[,c("region","freq")] |>
  left_join( Regions.Freq.byCards.Server |> 
               select(-n) |>
               pivot_wider(names_from = "server", values_from = "freq"  ), by = c("region") ) |>
  arrange(desc(freq)) |>
  gt( ) |>
  tab_header(
    title = "Region Play Rate",
...
## Aphelios/Ezreal (Tri-Beam Targon?)

```{r deckweek, layout="l-page"}
deck.of.week <- "Aphelios/Ezreal"

whichDecks <- LoR.Melt.Matches.RMD |>
  # filter( str_detect(playerDeck,"Jinx") )
