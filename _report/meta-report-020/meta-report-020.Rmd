---
title: "THE META REPORT NAME IS TOO LONG, TOO DAMN LONG (n°20)"
description: |
  Last week of Rise of the Underworld
base_url: https://llorr-stats.netlify.app
preview: "https://dd.b.pvp.net/latest/set5/en_us/img/cards/05BC194-full.png"
author:
...
#   creator: "@Maou_Legna"
params:
  prev:  "2021-07-11 21:00:00" #UTC tz / 'previous' week start
  start: "2021-08-18 21:00:00" #UTC tz / 'current' week start
...
meta() %>%
  ...
  meta_social(
    ...
    image = "https://dd.b.pvp.net/latest/set5/en_us/img/cards/05BC194-full.png",
    image_alt = "Badle tree",
    og_type = "website",
    og_author = "Legna",
...
```{r}
# iconRegion <- list.levels$iconRegion[-1]
iconRegion <- regionIcons[-1]
colorRegion.Balco <- list.levels$colorRegion.Balco[-1]
# regionIcons <- regionIcons[-1]
```


```{r process-tableFreq}
Region.Freq <- LoR.Melt.Matches.RMD %>%
  filter(!str_detect(factions,"Bandle")) %>%
  select(factions) %>%
...
RegionByRegion <- Region.Freq %>%
  ...
  geom_text(aes(label = round(freq*100,2) %>% ifelse(. >= 4,  . , "")),
...
Regions.Freq.byCards.Server <- LoR.Melt.Matches.RMD %>%
  ...
  map_df(.,~str_sub(.x, start=3,end=4) ) %>%
  mutate_all(~fct_recode(.,!!!recodeRegShort)) %>%
  add_column(server = LoR.Melt.Matches.RMD %>%
               filter(!str_detect(factions,"Bandle")) %>%
               pull(server)
             ) %>%
