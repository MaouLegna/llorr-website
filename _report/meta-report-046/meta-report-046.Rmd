...
params:
  ind: "MR046"
  patch: "Patch 3.02 - Week 1"
  title: "THE META REPORT NAME IS TOO LONG, TOO DAMN LONG (n°46)"
  description: "OOOOH WAAAAWEEE WAPAWOO!"
  #"Quarantaquattro Yuumi entrano a Bandle..."
  cardlurl: "https://dd.b.pvp.net/latest/set5/en_us/img/cards/05BC161-full.png"
  start: "2022-02-16 19:00:00" #UTC tz / 'current' week start
  end:   "2022-02-23 18:00:00" #UTC tz / 'current' week end
  nGrid: 10
  nDeck.CC:  20
  nDeck.Day: 12
  min.games: 5
...
# load Games DT
LoR.Temp.DT.RMD <- list.files(path = file.path("C:", "LlorR", "data", "raw"), pattern = glue("^(.*)Temp_DT_S12(.*)csv$"), full.names = T) |>
...
```{r account-info}
master.EU <- fread(file.path("C:","LlorR","data","raw","account","LoR_Master_S12_EU.csv"), header = T, na.strings = c("", NA), encoding = "UTF-8") |>
  filter(time <= as.POSIXct(params$end,"UTC"))

master.NA <- fread(file.path("C:","LlorR","data","raw","account","LoR_Master_S12_NA.csv"), header = T, na.strings = c("", NA), encoding = "UTF-8") |>
  filter(time <= as.POSIXct(params$end,"UTC"))

master.APAC <- fread(file.path("C:","LlorR","data","raw","account","LoR_Master_S12_APAC.csv"), header = T, na.strings = c("", NA), encoding = "UTF-8") |>
  filter(time <= as.POSIXct(params$end,"UTC"))

EU.master.found   <- master.EU[ !is.na(puuid), .N ]
NA.master.found   <- master.NA[ !is.na(puuid), .N ]
APAC.master.found <- master.APAC[ !is.na(puuid), .N ]

EU.master.ladder   <- NROW(master.EU)
NA.master.ladder   <- NROW(master.NA)
APAC.master.ladder <- NROW(master.APAC)
```
...
```{r check-value}
if (LoR.Melt.Temp.RMD[ is.na(playerDeck), .N ]!=0) print("If you are seeing this message - there are missing decks!")
if (LoR.Melt.Temp.RMD[ is.na(playerID), .N ]!=0)   print("If you are seeing this message - there are missing accounts!")
```
...
#   
tbl.temp    <- table_bythenumbers(LoR.Temp.DT.RMD)
tbl.master  <- table_bythenumbers(LoR.Match.DT.RMD)
tbl.diamond <- table_bythenumbers(LoR.Diamond.DT.RMD)

nGames    <- LoR.Melt.Temp.RMD[,.N]
nSeasonal <- LoR.Account.RMD[seasonal=="seasonal",.N]
```

Number of Players: **`r nSeasonal`** - Players from the last Seasonal Tournament

Number of (Ranked) matches analysed **`r format(nGames/2,scientific = F)`** or **`r format(nGames,scientific = F)`** games.
...
nGames_tbl <- tbl_merge(
  tbls = list(tbl.temp,tbl.master,tbl.diamond),
    tab_spanner = c("**Seasonal Players**","**Master**", "**~HighDiamond**")
  ) |>
  modify_footnote(
    update = label ~
...
Master <- LoR.Account.RMD[!is.na(master),.(activeShard,rank = master)]
Seasonal <- LoR.Account.RMD[seasonal=="seasonal",.(activeShard,rank = seasonal)]

Master |>
  bind_rows(Seasonal) |>
  select(activeShard,rank) |>
  mutate(across(everything(), str_to_title)) |>
  mutate(rank = factor(rank, levels = c("Master","Diamond","Seasonal") ) ) |>
...
nPlayer_tbl |>
  gtsave(glue::glue("images/{params$ind}-nPlayers.png")) |>
  invisible()
...
Region.Freq <- freq_by_faction(LoR.Melt.Temp.RMD)
```
...
Region.Freq.byCards <- freq_by_cards(LoR.Melt.Temp.RMD)
```
...
CC.table <- LoR.Melt.Temp.RMD |>
  tabyl(playerDeck, show_na = F) |>
  left_join(LoR.Melt.Temp.RMD |>
              tabyl(playerDeck,server) |>
              adorn_percentages("col"),by="playerDeck") |>
  slice_max(percent,n=params$nDeck.CC) |>
  rename(freq=percent) |>
  as.data.table()
```
...
CC.table |>
  ...
  ggplot(aes(x = playerDeck, y = freq, fill = freq)) +
  ...
  annotate(geom="text", x=3, y=0.05, col="black",label = glue::glue("n = {nGames}
                                                                    cumulative frequence = {scales::percent(CC.table[ ,sum(freq)],accuracy = 0.1)}") ) +
  theme_539() +
  labs(x = "Champions",
       y = "Play Rate",
       ...
       caption = glue::glue("{params$patch}
                            Ranked games from {params$start} UTC to {params$end} UTC
                            Games from players who attended the previous Seasonal Tournament
                            Source: Metadata of games collected with RiotGames API
                            Last Update: {now(tzone='UTC')}")
       ) +
  theme( 
...
CC.table |>
  ...
  reactablefmtr::add_source(element_text(
    glue::glue("Source: Metadata of games collected with RiotGames API - Games from players who attended the previous Seasonal Tournament - Last Update: {now(tzone='UTC')}")
    ), background_color = "whitesmoke", font_family = "Chivo")
```
:::
:::::
:::
...
Champion.deck.daily <- LoR.Melt.Temp.RMD |>
...
plot_daily(
  Champion.deck.daily,
  decks = fct_values,
  title = "Evolution of Play Rate over time - Seasonal Players",
  limit = .15
  ) -> meta_evo
...
```{r process-winRate}
WR.DT     <- win_rates(LoR.Melt.Temp.RMD,server = F)
WR.DT.Reg <- win_rates(LoR.Melt.Temp.RMD,server = T)
```
...
WR.DT |>
  filter(nGames >= params$min.games & WR >= 0.50 & playrate >= 0.001 & playrate < 0.01) |>
  slice_max(WR,n = 10) |>
...
```{r mu}
MUtbl <- match_ups(LoR.Melt.Temp.RMD)

TopCC <- CC.table |> 
  slice_max(freq,n=params$nGrid) |>
  pull(playerDeck)

MUtbl.TopCC <- tibble::tibble( 
...
grid <- matrix(MUtbl.TopCC$muWR, nrow = params$nGrid, ncol = params$nGrid, byrow = T)
diag(grid) <- NA
colnames(grid) <- TopCC
...
gtsave(MU.grid,glue::glue("images/{params$ind}-mugrid-gt.png"), vwidth = 1500, vheight = 1000) |> invisible()
```
...
gtWinning <- LoR.Melt.Temp.RMD[order(game_start_time_utc)][, counter := rowid(rleid(game_outcome)), by=c("playerID","playerDeck") ][ game_outcome == "win" ] |>
...
LoR.Melt.Temp.RMD[ cards.region.fix == "" , cards.region.fix:=NA   ]
LoR.Melt.Temp.RMD[ !is.na(cards.region.fix), cards:=cards.fix  ]

DeckWeight <- LoR.Melt.Temp.RMD |>
  tabyl(deck_code) |>
  select(-percent) |>
  rename(ndeck = n)
...
TableAllCards <- LoR.Melt.Temp.RMD |>
  # slice(1:10) |>
...
```{r missingCards}
missing.cards <- LoR.Card$cardCode[LoR.Card$name %!in% TableAllCards$card]

if (length(missing.cards)>0) {
  
LoR.Card |>
  as.data.table() |>
  filter( cardCode %in% missing.cards ) |>
  ...
  tibble(
    mag = "This is an happy week, everyone found home"
  ) |>
  gt() |>
    tab_options(
