...
params:
  ind: "MR032"
  patch: "Patch 2.19 - Week 1"
  title: "THE META REPORT NAME IS TOO LONG, TOO DAMN LONG (n°32)"
  description: "<This description is occupied by a sudden Ruined King>"
  cardlurl: "https://dd.b.pvp.net/latest/set5/en_us/img/cards/05PZ022T1-full.png"
  prev:  "2021-11-03 17:00:00" #UTC tz / 'current' previous week start
  start: "2021-11-10 17:00:00" #UTC tz / 'current' week start
  end:   "2021-11-17 17:00:00" #UTC tz / 'current' week end
  nGrid: 15
...
#   mutate(state = as.character(glue("{time} week / {label}")),
#          state = as.factor(state))

# fwrite(static.Region.Freq,"static.csv")

# static.Region.Freq$state <- factor(static.Region.Freq$state, ordered = T, levels = rev(levels(static.Region.Freq$state)) )

static.Region.Freq <- static.Region.Freq |>
  # mutate( freq = replace( freq, label == "~Plat+", -1*freq ) )
