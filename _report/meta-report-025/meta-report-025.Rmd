---
title: "THE META REPORT NAME IS TOO LONG, TOO DAMN LONG (n°25)"
description: |
  Patch 2.16 - Week 1 - Burn Meta Burn!
base_url: https://www.llorr-stats.com
preview: "https://dd.b.pvp.net/latest/set5/en_us/img/cards/05BC163-full.png"
author:
...
draft: FALSE
params:
  prev:  "2021-09-15 21:00:00" #UTC tz / 'current' previous week start
  start: "2021-09-22 21:00:00" #UTC tz / 'current' week start
  end:   "2021-09-29 21:00:00" #UTC tz / 'current' week end
...
`r kableExtra::text_spec("New:", color = "red")` as the Poppy Ziggs it's pretty much in it's entirely a burn deck, it not' called a RubinBait deck as there is no "no-burn" version.
...
nGames_tbl <- tbl_merge( 
  ...
  tab_header(
    ...
    subtitle = "Patch 2.16 first week / Master players"
    #'**************************************************************************** *
...
RegionByRegion <- Region.Freq |>
  ...
  labs(
    ...
    caption = glue::glue("Ranked games from {params$start} UTC to {params$end} UTC. Metadata of games collected with RiotGames API"),
    x = "Region",
    y = "Play Rate"
...
ChampionCombo.plot <- CC.table |>
  ...
  guides(fill = "none")  +
  theme_bw() +
  geom_text(aes(label=round(freq*100,2),size = 5, colour="red" ),
            face = "bold",
            color = "grey25",
            hjust = -0.5, size = 4,
            # vjust=-0.25
...
Champion.deck.daily <- LoR.Melt.Matches.RMD |>
  ...
  .[ , cumFreq := (cumsum(num_cc)/num_cc)/.I , by=champions ] |>
  select(!num_cc) |>
...
#   filter(str_detect(userID,"RubinZoo")) |>
#   tabyl(player)

# WR.DT |>
#   filter(str_detect(player,"Rubin"))
...
Top10CC <- CC.table |> arrange(desc(freq)) %>% slice_head(.,n = mu) |> select(player) |> pull() |> sort()
...
This week deck choice is fulfilling [a promise I did on Reddit](https://www.reddit.com/r/LegendsOfRuneterra/comments/pvts7r/comment/hecqxs5/?utm_source=share&utm_medium=web2x&context=3)

```{r}
deck.of.week <- "Lucian / Zed"
WR.deck.week <- LoR.Melt.Matches.RMD |>
  filter(player == deck.of.week ) |>
  summarise( WR = mean(game_outcome=="win") ) %>%
  pull() %>%
  scales::percent(.,accuracy = 0.1)
```

As cards are released quite frequently it's obvious that some decks takes a seat back, sometimes because of power-creep, but sometimes, people just don't have the time and want to focus on the new toys.

With the release of BandleCity while there are of course some top-tier deck that are above the rest I don't think I'm in the minority by saying this is overall an healthy meta with lots and lots of option.

This week I want to highlight something old, maybe so old that may result fresh for some players: `r deck.of.week`

With a respectable win rate of `r WR.deck.week` (even accounting the small sample size), this wants to be a reminder that are often forgotten options are still worth exploring.

Note: I'm not Master but I'm currently having fun and discrete success with Nightfall, for the lower ranks like me my message this week is: "Play your deck and have fun! This is a good moment to let your deck building prowess carry you."
...
SubDeck_deck_code <- LoR.Melt.Matches.RMD |>
  filter(player == deck.of.week ) |>
  filter( game_start_time_utc > as.POSIXct("2021-09-20 15:33:00", tz = "UTC") ) |>
  pull(deck_code)
...
```{r ggplotly-LMI,echo=FALSE, fig.width=12, fig.height=8, layout="l-body-outset"}
fig <- DT.Aggreation |>
  slice_max(LMI,n = 50,with_ties = F) %>%
  mutate_if(is.numeric, funs(round(., 4)) ) |>
  ggplot( aes(x=wr_ind, y=freq_ind, size = LMI^2, color = tier, text=tooltip)) +
...
LoR.Melt.Matches.RMD[order(game_start_time_utc)][, counter := rowid(rleid(game_outcome)), by=c("userID","player") ][ game_outcome == "win" ] |>
  # filter( server=="asia" & userID == "Ryo 6192" & player ==	"Senna / Veigar (BC/SI)" )
  group_by(userID) |>
  slice_max(counter,n=1) |>
  ungroup() |>
  group_by(server) |>
  slice_max(counter,n=3) |>
  left_join(LoR.Account.RMD[ ,c("RiotID","gameName") ]|>distinct(RiotID,.keep_all = T), by =c("userID"="RiotID") ) |>
  select(server,gameName,counter,player,deck_code) |>
  mutate( server = str_to_title(server) ) |>
  group_by(server) |>
  mutate( ranking = rank(-counter,ties.method = "min" ) ) |>
  ungroup() %>% 
  mutate(counter_icons = dplyr::case_when(
    ranking == 1 ~ "trophy",
    ranking == 2 ~ "medal",
    ranking == 3 ~ "minus",
    TRUE ~ "other"
  ...
  reactable(
            ...
            columns = list(
              ...
              server = colDef(name = "Server"),
              gameName = colDef(name = "Player",minWidth = 200),
              player = colDef(name = "Archetype",minWidth = 200),
              counter_icons = colDef(show = FALSE),
              ranking = colDef(show = FALSE),
              counter = colDef(name = "Consecutive Wins",cell = icon_sets(., icon_ref = "counter_icons", icon_size = 15, colors = "#67a9cf") )
              ),
             rowStyle = JS("function(rowInfo) {
                           if (rowInfo.level > 0) {
                           return { background: 'whitesmoke', borderLeft: '2px solid #ffa62d' }
