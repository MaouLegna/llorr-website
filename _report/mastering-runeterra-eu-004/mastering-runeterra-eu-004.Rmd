...
plot.LMIbasic <- LMI.DT |>
  ...
  mutate( LMI = LMI*100 ) |>
  ggplot( aes(x=playrate, y=WR, size = LMI, color = tier, text=tooltip_mini)) +
...
downloadthis::download_this(
  plot.LMIbasic,
  button_label = "Download Top10 LMI plot as .png",
  button_type = "primary",
  has_icon = TRUE,
  icon = "fa fa-save",
  self_contained = FALSE
...
plot.LMIfull <- LMI.DT |>
  ...
  mutate( LMI = LMI*100 ) |>
  ungroup() |>
  ggplot( aes(x=PR_ind, y=WR_ind, size = LMI, color = tier, text=tooltip)) +
  geom_point(alpha=0.6) +
  scale_size(range = c(1, 15), name="LMI") +
  theme_539() +
  geom_label_repel( 
