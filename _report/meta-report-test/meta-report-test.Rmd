...
region.freq.history
  ...
  add_column( sample_size = map2_dbl(.x = calendar$start, .y = calendar$end, .f = ~NROW( LoR.Ranked.RMD %>% 
                                                            dplyr::filter( game_start_time_utc >= .x & game_start_time_utc < .y )
                                                          ) ) ) %>%
  rowwise() %>%
