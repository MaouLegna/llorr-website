...
params:
  ind: "MR045"
  patch: "Patch 3.10 - Week 3"
  title: "THE META REPORT NAME IS TOO LONG, TOO DAMN LONG (n°45)"
  description: "Runeterra Tensei 6: A Curious Journey Redux"
  #"Quarantaquattro Yuumi entrano a Bandle..."
  cardlurl: "https://dd.b.pvp.net/3_2_0/set5/en_us/img/cards/05BC029-full.png"
  start: "2022-02-09 18:00:00" #UTC tz / 'current' week start
  end:   "2022-02-16 18:00:00" #UTC tz / 'current' week end
  nGrid: 10
...
# load Games DT
LoR.Match.DT.RMD <- list.files(path = file.path("C:", "LlorR", "data", "raw"), pattern = glue("^(.*)Match_DT_S11(.*)csv$"), full.names = T) |>
...
# Read Diamond DT
LoR.Diamond.DT.RMD <- list.files(path = file.path("C:", "LlorR", "data", "raw"), pattern = glue("^(.*)Diamond_DT_S11(.*)csv$"), full.names = T) |>
  str_subset(pattern = "Keys",negate = T) |>
  map_dfr(data.table::fread,header = T, na.strings = c("", NA), encoding = "UTF-8") |>
  mutate(server = if_else(server %in% c("sea","asia"),"apac",server) )

# Read Diamond DT
LoR.Seasonal.DT.RMD <- list.files(path = file.path("C:", "LlorR", "data", "raw"), pattern = glue("^(.*)Seasonal(.*)csv$"), full.names = T) |>
  str_subset(pattern = "Keys",negate = T) |>
  map_dfr(data.table::fread,header = T, na.strings = c("", NA), encoding = "UTF-8") |>
  mutate(server = if_else(server %in% c("sea","asia"),"apac",server) ) |>
  filter(game_mode == "SeasonalTournamentLobby") |>
  filter(game_start_time_utc >= as.POSIXct("2022-02-11 00:00:00") )
```

```{r fix-account}
setDT(LoR.Account.RMD)
LoR.Account.RMD[, num_obs := Reduce(`+`, lapply(.SD,function(x) !is.na(x)))]
LoR.Account.RMD <- LoR.Account.RMD |>
  group_by(puuid,activeShard) |>
  slice_max(num_obs,n=1,with_ties = F) |>
  ungroup() |>
  as.data.table()
```
...
```{r account-info}
master.EU <- fread(file.path("C:","LlorR","data","raw","account","LoR_Master_S11_EU.csv"), header = T, na.strings = c("", NA), encoding = "UTF-8") |>
  filter(time <= as.POSIXct(params$end,"UTC"))

master.NA <- fread(file.path("C:","LlorR","data","raw","account","LoR_Master_S11_NA.csv"), header = T, na.strings = c("", NA), encoding = "UTF-8") |>
  filter(time <= as.POSIXct(params$end,"UTC"))

master.APAC <- fread(file.path("C:","LlorR","data","raw","account","LoR_Master_S11_APAC.csv"), header = T, na.strings = c("", NA), encoding = "UTF-8") |>
  filter(time <= as.POSIXct(params$end,"UTC"))

namesList.EU.RMD   <- master.EU[ !is.na(puuid), .N ]
namesList.NA.RMD   <- master.NA[ !is.na(puuid), .N ]
namesList.APAC.RMD <- master.APAC[ !is.na(puuid), .N ]

EU.ladder   <- NROW(master.EU)
NA.ladder   <- NROW(master.NA)
APAC.ladder <- NROW(master.APAC)
```
...
LoR.Melt.Master.RMD <- LoR.Match.DT.RMD |>
  ...
  # filter( game_start_time_utc %within% interval(as.POSIXct(params$start,"UTC")-days(7),as.POSIXct(params$end,"UTC")-days(7)) ) |>
  filter(game_start_time_utc %within% interval(as.POSIXct(params$start,"UTC"),as.POSIXct(params$end,"UTC")) ) |>
  filter(puuid_1 %in% c(master.APAC$puuid,master.EU$puuid,master.NA$puuid) |
           puuid_2 %in% c(master.APAC$puuid,master.EU$puuid,master.NA$puuid) ) |>
  distinct(match_key,puuid_1,.keep_all = T) |>
...
```{r fix-data-master}
LoR.Melt.Master.RMD <- LoR.Melt.Master.RMD |>
  filter(!is.na(playerID) & !is.na(opponentID))
```
...
LoR.Melt.Diamond.RMD <- LoR.Melt.Diamond.RMD |>
  filter(!is.na(playerID) & !is.na(opponentID))
```

```{r}
LoR.Melt.Seasonal.RMD <- LoR.Seasonal.DT.RMD |>
  rename( playerPuuid_1 = puuid_1, playerPuuid_2 = puuid_2 ) |>
  mutate( opponentPuuid_1 = playerPuuid_2, opponentPuuid_2 = playerPuuid_1 ) |>
  mutate( opponent_deck_code_1 = deck_code_2, opponent_deck_code_2 = deck_code_1 ) |>
  pivot_longer(cols = c(ends_with("_1"),ends_with("_2")),
  ...
  left_join(
    ...
    by=c("opponent_deck_code"="deck_code")) |>
  # Add Player data
  left_join(LoR.Account.RMD[,.(puuid,playerID=gameName)]   ,by=c("playerPuuid"="puuid")) |>
  left_join(LoR.Account.RMD[,.(puuid,opponentID=gameName)] ,by=c("opponentPuuid"="puuid"))

# LoR.Melt.Seasonal.RMD |>
#   filter(is.na(playerID) & !is.na(opponentID))
# 
# LoR.Melt.Seasonal.RMD <- LoR.Melt.Seasonal.RMD |>
#   filter(!is.na(playerID) & !is.na(opponentID))
```
...
if (any(c(LoR.Melt.Diamond.RMD[is.na(playerDeck), .N ],
          ...
          LoR.Melt.Seasonal.RMD[is.na(playerID),   .N ],
          LoR.Melt.Seasonal.RMD[is.na(playerDeck), .N ]
...
nMaster  <- length(c(master.APAC$puuid,master.EU$puuid,master.NA$puuid))
# nDiamond <- LoR.Account.RMD[ master == "diamond" , .N ]
 
# LoR.Melt.Master.RMD |>
#   filter(playerPuuid %in% c(master.APAC$puuid,master.EU$puuid,master.NA$puuid)  ) |>
#   distinct(match_key,playerPuuid) |>
#   NROW() -> gamesMaster
# 
# LoR.Melt.Diamond.RMD |>
#   filter( playerPuuid %in% LoR.Account.RMD[ master == "diamond" , puuid ]  ) |>
#   distinct(match_key,playerPuuid) |>
#   NROW() -> gamesDiamond
```

Number of Players: **`r nMaster`** Master
...
master.EU |>
  bind_rows(master.NA) |>
  bind_rows(master.APAC) |>
  add_column(master="master") |>
  select(activeShard,rank=master) |>
  mutate(across(everything(), str_to_title)) |>
  # mutate(rank = factor(rank, levels = c("Master","Diamond") ) ) |>
  tbl_summary(by = activeShard,
               label = list(rank = "Player Rank")) |>
  modify_header(label ~ "**Shard/Server**") |>
  bold_labels() |>
  # modify_caption( glue::glue("{params$patch} - Players Info") ) |>
  as_gt() |>
  tab_options(
...
nGames_tbl <- tbl_merge(
  ...
  tab_footnote(
    footnote = md(glue::glue(
      "EU Master {master.EU[,.N]}
      NA Master {master.NA[,.N]}
      APAC Master {master.APAC[,.N]}"
      )),
...
LineUp.DT <- LoR.Melt.Seasonal.RMD |>
  group_by(playerID,playerPuuid,server) |>
...
LineUp.DT <- LineUp.DT |>
  ...
  ungroup() |>
  right_join(LineUp.DT, by = "playerPuuid") |>
  #' quality filter
  filter( !is.na(deck_1) ) |>
...
Game.Results <- LoR.Melt.Seasonal.RMD |>
  ...
  ungroup() |>
  left_join(LineUp.DT,by=c("server","playerPuuid")) |>
...
winlosstbl <- Game.Results |>
  ...
  tab_header(
    ...
    subtitle = md("Game results from the top32 players of the Magic Disadventures Seasonal Tournament") 
  ) |>
  tab_source_note(source_note = md("Metadata of games collected with RiotGames API")) |>
  gtExtras::gt_theme_538()

gtsave(winlosstbl,glue::glue("images/{params$ind}-winloss.png")) |> invisible()
...
react.LU.top32 <- top32tbl |>
  left_join(LoR.Account.RMD[ ,.(puuid,gameName) ],by=c("playerPuuid"="puuid") ) |>
  select(-playerPuuid) |>
  ...
  reactablefmtr::add_title("Top32 Players' Deckcodes", background_color = "white", font_family = "Helvetica") |>
  reactablefmtr::add_subtitle(c("Magic Disadventures Playoff - Top32 LineUps"), background_color = "white", font_family = "Roboto") |>
  reactablefmtr::add_source(glue::glue("As not all decks has being played I can't provide a complete coverage of the lineUps but {deckFound}/{32*3*3} decks overall.
                                       Source: Metadata of games collected with RiotGames API"), background_color = "white", font_family = "Chivo")

react.LU.top32

reactablefmtr::save_reactable(input = react.LU.top32,output = glue::glue("images/{params$ind}-top32-decks.png")) |> invisible()
```
:::
...
Highlighting the play-rates of most played[^2] decks over time.
...
setindex(LoR.Melt.Master.RMD, NULL);
...
# setindex(LoR.Melt.Diamond.RMD, NULL);
...
plot_daily(
  ...
  start = as.POSIXct(params$start,"UTC"),
  end   = as.POSIXct(params$end,"UTC"),
  limit = .15 )
```
:::
:::::
...
[^2]: at the moment of the last game
...
Top Win rates of the top10 best performing least played combination of champions. Play rate $\in [0.1%,1%)$ [^3]
...
[^3]: Min number of games 50, during the times a meta/ladder just changed.
