...
params:
  ind: "MR045"
  patch: "Patch 3.00 - Week 6"
  title: "THE META REPORT NAME IS TOO LONG, TOO DAMN LONG (n°45)"
  description: "Runeterra Megami Tensei 5: A Curious Journey Redux"
...
EU.ladder   <- length(lorR::lor_leaderboard("europe",names = T))
NA.ladder   <- length(lorR::lor_leaderboard("americas",names = T))
APAC.ladder <- length(lorR::lor_leaderboard("sea",names = T))
```
...
nMaster  <- sum(masterEU[ !is.na(puuid), .N ],masterNA[ !is.na(puuid), .N ],masterAPAC[ !is.na(puuid), .N ])
nDiamond <- LoR.Account.RMD[ master == "diamond" , .N ]

LoR.Melt.Master.RMD |>
  filter( playerPuuid %in% LoR.Account.RMD[ master == "master" , puuid ]  ) |>
...
Number of Players: **`r nMaster`** Master / **`r nDiamond`** HighDiamond [^1]
...
nGames_tbl <- tbl_merge( 
  ...
  tab_footnote(
    footnote = md(glue::glue(
      "EU Master {masterEU[,.N]}/{EU.ladder}
      NA Master {masterNA[,.N]}/{NA.ladder}
      APAC Master {masterAPAC[,.N]}/{APAC.ladder}"
      )),
...
<!-- Player to Games Ratio:  **`r round(gamesMaster/nMaster,1)`** at Master / **`r round(gamesDiamond/nDiamond,1)`** at Diamond ^[ Approximation, number of games from Master player to Master player ] -->
...
react.LU.top32 <- top32tbl |>
  ...
  reactablefmtr::add_subtitle(c("Magic Disadventure Playoff - Top32 LineUps"), background_color = "white", font_family = "Roboto") |>
...
reactablefmtr::save_reactable(input = react.LU.top32,output = glue::glue("images/{params$ind}-top32-decks.png"))
```
:::
