...
gtSeasonal <- tibble( server = c("asia","europe","americas"),
  ...
  tab_header(
    ...
    subtitle = glue::glue("Coverage reached regarding the {params$description}")
  ) |>
  tab_source_note(
...
gtSeasonal |>
  gtsave( glue::glue("./images/seasonal/{params$ind}-coverage.png") )
...
react.LU.Stat |>
  reactablefmtr::save_reactable("./images/seasonal/{params$ind}-deck.png")
```
...
react.LU.Full |>
  reactablefmtr::save_reactable("./images/seasonal/{params$ind}-LU.png")
```
:::
:::::
:::
...
Some basic data regarding the regions play-rates
...
ggtop32.ASIA <- top32tbl.CC  |>
  ...
  left_join( data_regions, by = c("regionRef"="nameRef") ) |>
  ...
  # scale_fill_manual("legend", values = set_names(data_regions$nameRef[-1], data_regions$colorRegion[-1]) ) +
  theme_539() +
  labs( x = "Champion",
        y = "#Decks",
...
react.LU.top32 <- top32tbl |>
  ...
  reactablefmtr::add_subtitle(params$description, background_color = "transparent", font_family = "Roboto") |>
  reactablefmtr::add_source(element_text(glue::glue("Source: Source: Metadata of games collected with RiotGames API")), background_color = "transparent", font_family = "Chivo")

react.LU.top32

react.LU.top32 |>
  reactablefmtr::save_reactable("./images/seasonal/{params$ind}-top32.png")
```
:::
:::::
...
CC.table.S |>
  select(-n) |>
  pivot_longer(
    cols = freq:europe,values_to = "seasonal",names_to = "server"
    ) |>
  left_join(
    CC.table |>
      select(-n) |>
      pivot_longer(
        cols = freq:europe,values_to = "ladder",names_to = "server"),
