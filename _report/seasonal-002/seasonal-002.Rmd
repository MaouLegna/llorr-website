...
```{r }
gtLMI <- LMI.DT |>
  select(deck,LMI,tier,WR,playrate,meanBan,lWR) |>
  mutate( 
    # diffWR = WR-lWR,
    tier = case_when( 
          LMI >= 0.975 ~ 4,
          0.85 <= LMI & LMI < 0.975 ~ 3,
          0.60 <= LMI & LMI < 0.85 ~ 2,
          LMI < 0.60 ~ 1
        ),
    LMI = round(LMI *100,0)
    # tier = case_when( 
    #       x >= 0.975 ~ "Tier0",
    #       0.85 <= x & x < 0.975 ~ "Tier1",
    #       0.60 <= x & x < 0.85 ~ "Tier2",
    #       x < 0.60 ~ "Tier3"
    #     )
    # tier = factor(tier, levels = c("Tier3","Tier2","Tier1","Tier0"), ordered = T),
    # nTier = as.numeric(tier)
    ) |>
  # relocate(diffWR, .after = meanBan) |>
  relocate(tier,.after = "deck") |>
  arrange(desc(LMI)) |>
  gt() |>
  # gtExtras::gt_color_rows(WR:lplayrate) |>
  ...
  gtExtras::gt_hulk_col_numeric(
    tier,
    # type = "discrete",
    # palette = c("#ffffff", "##00FF00", "##00FFFF", "##FFFF00")
    # domain = c(1:4)
   ) |>
  text_transform(
...
gtsave(gtLMI,glue::glue("./images/seasonal/S2021-10-09-LMI-table.png") ) |> invisible()
```
