...
# Game.Result.EU |>
#   filter( str_detect(playerID,"Balco") )
...
gtSeasonal <- tibble( server = c("asia","europe","americas"),
        players = c(glue::glue("{ifelse(length(nSeasonal[server=='asia',n])==0,0,nSeasonal[server=='asia',n])}/(?)"),glue::glue("{ifelse(length(nSeasonal[server=='europe',n])==0,0,nSeasonal[server=='europe',n])}/1024(?)"),glue::glue("{ifelse(length(nSeasonal[server=='americas',n])==0,0,nSeasonal[server=='europe',n])}/1024(?)"))
        ) |>
...
gtSeasonal |>
 tab_options(
  ...
  gtsave( glue::glue("./images/seasonal/{params$ind}-coverage-player.png") )
...
Game.Result.EU <- LoR.Melt.Seasonal |>
  ...
  mutate( round = case_when(
    ...
    timeBlock > (with_tz(force_tz(timeBlock.3.start,"CET"),"UTC")+minutes(90)) & timeBlock <= (with_tz(force_tz(timeBlock.3.end,"CET"),"UTC")+minutes(90)) ~ 3, # 14:05
    timeBlock > (with_tz(force_tz(timeBlock.4.start,"CET"),"UTC")+minutes(90)) & timeBlock <= (with_tz(force_tz(timeBlock.4.end,"CET"),"UTC")+minutes(90)) ~ 4, # 15:10
    timeBlock > (with_tz(force_tz(timeBlock.5.start,"CET"),"UTC")+minutes(90)) & timeBlock <= (with_tz(force_tz(timeBlock.5.end,"CET"),"UTC")+minutes(90)) ~ 5, # 16:15
    
    timeBlock > (with_tz(force_tz(timeBlock.6.start,"CET"),"UTC")+minutes(90)) & timeBlock <= (with_tz(force_tz(timeBlock.6.end,"CET"),"UTC")+minutes(90)) ~ 6, # 17:45
    timeBlock > (with_tz(force_tz(timeBlock.7.start,"CET"),"UTC")+minutes(90)) & timeBlock <= (with_tz(force_tz(timeBlock.7.end,"CET"),"UTC")+minutes(90)) ~ 7, # 18:50
    timeBlock > (with_tz(force_tz(timeBlock.8.start,"CET"),"UTC")+minutes(90)) & timeBlock <= (with_tz(force_tz(timeBlock.8.end,"CET"),"UTC")+minutes(90)) ~ 8, # 19:55
    timeBlock > (with_tz(force_tz(timeBlock.9.start,"CET"),"UTC")+minutes(90)) ~ 9, # 21:00
    )
  ) |>
  # number of won games in the match
  mutate( nWin = str_count(result,"win") ) |>
...
gtMatch |>
 tab_options(
  ...
  gtsave( glue::glue("./images/seasonal/{params$ind}-coverage-match.png") )
...
```{r double-faction}
# tibble(
#   faction_1 = rep(data_regions$nameRef,each = 10),
#   faction_2 = rep(data_regions$nameRef,time = 10)
#   ) |>
#   left_join(
#     LoR.Melt.Seasonal |>
#       select(factions) |>
...
```{r create-top32-ASIA, fig.height=12, fig.width=16}
...
#   mutate(img = map_chr(.x = img, ~card_url_path(.x,full=T) )) |>
#   left_join( data_regions, by = c("regionRef") ) |>
#   ggplot(aes(reorder(name, n),n,fill=regionRef)) +
#   geom_bar(position="dodge",stat = "identity", color = "black") +
#   scale_y_continuous(breaks = seq(0,12,2), sec.axis = sec_axis(~(./(as.data.table(top32tbl)[ ,.N, by=server][server=="asia",N]))*100, name = "PlayRate (%)",breaks = seq(0,100,20))) +
#   ggimage::geom_image(aes(x = name, image = img), y = 0, size = 0.075,inherit.aes = FALSE) +
#   coord_flip() +
...
#                      The top players are selected from those with the score scores in Matchss wins.
...
react.LU.top32 <- top32tbl |>
  ...
  reactable(
    ...
    columns = list(
      ...
      cumMatchWin = colDef(name = "#Matches Wins", aggregate = "unique")
      ),
...
df <- tibble(x = 0.01, y = 0.12, tb = list(tb ))
