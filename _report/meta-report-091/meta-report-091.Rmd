...
params:
  ind: "MR091"
  season: "S17"
  patch: "Patch 3.21 - Week 4"
  title: "THE META REPORT NAME IS TOO LONG, TOO DAMN LONG (n°91)"
  description: "Riot Hybernation is over this week! Patch Notes Waiting Room!"
  cardlurl: "https://dd.b.pvp.net/latest/set6cde/en_us/img/cards/06RU006T5-full.png"
  start: "2022-12-28 18:00:00" #UTC tz / 'current' week start
  end:   "2023-01-04 18:00:00" #UTC tz / 'current' week end
  nGrid: 10
  nDeck_cc:  20
  nDeck_day: 12
  min_games: 10
...
source(file.path("C:", "LlorR", "scripts", "functions", "lor_constants.R"))
source(file.path("C:", "LlorR", "scripts", "functions", "load_cards_info.R"))
...
```{r load-data}
# load DeckDT
LoR_Deck       <- data.table::fread(file.path("C:", "LlorR", "data", "raw", glue::glue("LoR_DECK_{params$season}.csv")), na.strings = c("", NA), colClasses = "character")

# load Games DT
LoR_Match_DT <- list.files(path = file.path("C:", "LlorR", "data", "raw"), pattern = glue("^(.*)Match_DT_{params$season}(.*)csv$"), full.names = T) |>
...
LoR_Diamond_DT <- list.files(path = file.path("C:", "LlorR", "data", "raw"), pattern = glue("^(.*)Diamond_DT_{params$season}(.*)csv$"), full.names = T) |>
  map_dfr(data.table::fread, header = T, na.strings = c("", NA), encoding = "UTF-8") |>
  mutate(server = if_else(server %in% c("sea", "asia"), "apac", server) ) |>
  filter(game_start_time_utc %within% interval(as.POSIXct(params$start, "UTC"), as.POSIXct(params$end, "UTC")) )

LoR_Account <- list.files(path = file.path("C:", "LlorR", "data", "raw"), pattern = glue("^(.*)ACCOUNT(.*)csv$"), full.names = T) |>
...
```{r deck-fix}
LoR_Deck <- update_and_save_deck(
  LoR_Deck,
  LoR_Match_DT |>
    bind_rows(LoR_Diamond_DT)
    # bind_rows(LoR_Temp_DT)
)
LoR_Deck[ !is.na(archetype_pretty), archetype:=archetype_pretty ]
```

```{r prepare-data}
LoR_Melt_Master <- LoR_Match_DT |>
  filter(game_type == "Ranked") |>
  melt_match(LoR_Deck, LoR_Account)

LoR_Melt_Diamond <- LoR_Diamond_DT |>
  filter(game_type == "Ranked") |>
  melt_match(LoR_Deck, LoR_Account)
```
...
```{r nGames-data}
nGames_master  <- NROW(LoR_Melt_Master)
nMaster   <- LoR_Account[master=="master", .N]

nGames_diamond  <- NROW(LoR_Melt_Diamond)
nDiamond  <- LoR_Account[master=="diamond", .N]
```
...
nGames_tbl <- table_by_the_numbers_double(LoR_Match_DT, LoR_Diamond_DT, LoR_Account)
...
nPlayers <- table_accounts(LoR_Account,"rank")
...
region_freq <- freq_by_faction(LoR_Melt_Master)
```
...
region_freq_by_cards <- freq_by_cards(LoR_Melt_Master)
```
...
cc_table_master <- LoR_Melt_Master |>
  tabyl(playerDeck, show_na = F) |>
  left_join(LoR_Melt_Master |>
              tabyl(playerDeck, server) |>
              adorn_percentages("col"), by="playerDeck") |>
  as.data.table()

cc_table_diamond <- LoR_Melt_Diamond |>
  tabyl(playerDeck, show_na = F) |>
  left_join(LoR_Melt_Diamond |>
              tabyl(playerDeck, server) |>
              adorn_percentages("col"), by="playerDeck") |>
  as.data.table()
...
champion_combination |>
  ...
  labs(
    ...
    caption = glue::glue("{params$patch}
                         Ranked games from {params$start} UTC to {params$end} UTC
                         Decks are chosen from the union of top {params$nDeck_cc} from Master player and ~HighDiamond resulting in {length(top_deck)} displayed
                         Source: Metadata of games collected with RiotGames API
                         Last Update: {now(tzone='UTC')}")
  ) +
  theme( 
...
hourly_meta_plot_line <- plot_playrate_by_time(LoR_Melt_Master,prev_days = 7,max_top_pr = 12)
...
hourly_meta_plot <- plot_hourly_meta_deck(data = LoR_Melt_Master,prev_days = 7,max_top_pr = 12)
...
hourly_meta_plot_abs <- plot_hourly_meta_deck(data = LoR_Melt_Master,prev_days = 7,max_top_pr = 12,absolute = T)
...
```{r process-winRate}
WR_tbl     <- win_rates(LoR_Melt_Master, server = F)
WR_tbl_Reg <- win_rates(LoR_Melt_Master, server = T)
```
...
reactable_report(data = LoR_Melt_Master,group = "meta")
```
:::
...
reactable_report(data = LoR_Melt_Master,group = "underdog")
```
:::
:::::
:::
...
MU_tbl <- match_ups(LoR_Melt_Master)

grid_source_note <- md(glue::glue("MatchUp values from Ranked games of Master rank players. Order of the Archetypess based on the playrate.
                                  Source: Metadata of games collected with RiotGames API") )

gtMU_grid <- mu_grid(mu_data = MU_tbl, dim_grid = params$nGrid, 
                     games_data = LoR_Melt_Master, prev_days = 7, 
...
gt_winning <- table_winning_streak(LoR_Melt_Master, LoR_Account, "master")
...
LoR_Melt_Master[ !is.na(cards_fix), cards:=cards_fix  ]

DeckWeight <- LoR_Melt_Master |>
  tabyl(deck_code) |>
  select(-percent) |>
  rename(ndeck = n)
...
LoR_Card |>
  as.data.table() |>
  filter( cardCode %in% LoR_Card$cardCode[LoR_Card$name %!in% TableAllCards$card] )  -> missing_cards

if (NROW(missing_cards)>0) {
