...
params:
  ind: "MR038"
  patch: "Patch 2.22 - Week 3"
  title: "THE META REPORT NAME IS TOO LONG, TOO DAMN LONG (n°38)"
  description: ""
  cardlurl: "https://dd.b.pvp.net/latest/set5/en_us/img/cards/05IO004-full.png"
  prev:  "2021-11-15 18:00:00" #UTC tz / 'current' previous week start
  start: "2021-12-22 18:00:00" #UTC tz / 'current' week start
  end:   "2021-12-29 18:00:00" #UTC tz / 'current' week end
  nGrid: 10
...
masterASIA <- fread(file.path("C:","LlorR","data","raw","account","LoR_Master_S11_AS.csv"), header = T, na.strings = c("", NA), encoding = "UTF-8") |>
  filter(time <= as.POSIXct(params$end,"UTC"))
...
```{r nGames-data}
# minrow_live <- min(which(LoR.Match.DT.RMD[,game_start_time_utc >= as.POSIXct(params$start,"UTC")]))
minrow_live <- LoR.Match.DT.RMD |>
  select( game_start_time_utc ) |>
  mutate( row = row_number() ) |>
  filter( game_start_time_utc >= as.POSIXct(params$start,"UTC") ) |>
  slice_head(n=1) |>
  pull(row)
  # I don't have to take the row with the min time but the first row index
  # filter( game_start_time_utc == min(game_start_time_utc) ) |>
  
# maxrow_live <- max(which(LoR.Diamond.DT.RMD[,game_start_time_utc  < as.POSIXct(params$end)]  ))
maxrow_live <- LoR.Match.DT.RMD |>
  select( game_start_time_utc ) |>
  mutate( row = row_number() ) |>
  filter( game_start_time_utc < as.POSIXct(params$end,"UTC") ) |>
  slice_tail(n=1) |>
  pull(row)

max_time <- LoR.Melt.Master.RMD |>
...
LoR.Match.DT.RMD |>
  ...
  mutate( game_type = case_when(
    status==404   ~ "Friendly",
    game_mode=="Power_XP1" ~ "PathOfChampion",
  ...
  ##########################################################################################
  mutate( game_start_time_utc = replace( game_start_time_utc, game_type %in% "Friendly",  as.POSIXct(params$start, tz = "UTC")+days(3) ) ) |>
  filter( game_start_time_utc %within% interval(as.POSIXct(params$start, tz = "UTC"), as.POSIXct(params$end, tz = "UTC") ) ) |>
  mutate( game_type = fct_other(game_type, keep = c("Ranked","Friendly","PathOfChampion")) |> fct_infreq() ) |>
  select(Status=game_type,Server=server) |>
  tbl_summary() -> tbl1

LoR.Melt.Master.RMD |>
  distinct(match_key,server) |>
  select(Server=server) |>
  tbl_summary() -> tbl2

tbl_merge( tbls = list(tbl1, tbl2),
           tab_spanner = c("**All Games**", "**Ranked**") ) -> merge.tbl.master
```

```{r nGames-data-diamond}
# minrow_live <- min(which(LoR.Diamond.DT.RMD[,game_start_time_utc >= as.POSIXct(params$start)] )) # base buth more demanding
minrow_live <- LoR.Diamond.DT.RMD |>
  select( game_start_time_utc ) |>
  mutate( row = row_number() ) |>
  filter( game_start_time_utc >= as.POSIXct(params$start,"UTC") ) |>
  slice_head(n=1) |>
...
# maxrow_live <- max(which(LoR.Diamond.DT.RMD[,game_start_time_utc  < as.POSIXct(params$end)]  ))
maxrow_live <- LoR.Diamond.DT.RMD |>
  select( game_start_time_utc ) |>
  mutate( row = row_number() ) |>
  filter( game_start_time_utc >= as.POSIXct(params$start,"UTC") ) |>
  slice_tail(n=1) |>
...
Number of (Ranked) matches analysed **`r format(nMatches.extra,scientific = F)`** or **`r format(nMatches.extra*2,scientific = F)`** games. / **Master** Players
...
Regions.Freq.byCards |>
  ...
  geom_bar_pattern(
    ...
    pattern         = 'image',
    pattern_type    = 'none',
    fill            = Regions.Freq.byCards$colorRegion,
    colour          = 'black',
    pattern_scale   = -2,
    pattern_filter  = 'point',
    pattern_gravity = 'east'
...
fct_values <- LoR.Melt.Master.RMD |>
 mutate(topN = fct_lump_n(playerDeck,n = m) |> fct_infreq() ) |>
  pull(topN) |> 
  levels()
```
...
```{r plot-diamond-day-by-day, fig.width=12, fig.height=8}
meta_evo <- ggplot(
  data = Champion.deck.daily |> filter(champions %in% fct_values[-1]),
  aes(game_start_time_utc, cumFreq, group = champions) ) +
  geom_line(
  ...
  geom_label_repel(
    ...
    family = "Helvetica", size = 3.5, direction = "y", force = 3, hjust = 1, segment.size = .7, segment.alpha = .5, segment.linetype = "dotted", box.padding = .4, segment.curvature = -0.1, segment.ncp = 3,
    segment.angle = 15, bg.color = "grey50", fill = "#f0f0f0", abel.padding = 0, label.size = 0
  ) +
  scale_x_datetime(
    date_minor_breaks = "1 day",
    limits = c(as.POSIXct(params$start, tz="UTC"), as.POSIXct(params$end, tz="UTC")+hours(23))
  ...
  scale_color_manual(
    values = rcartocolor::carto_pal(12, "Safe")
    # values = c(rcartocolor::carto_pal(n = 10, name = "Vivid")[1:10-1], "grey50")
  ) +
  labs(x = "Date",
       y = "PlayRate",
       title = "Evolution of ~HighDiamond Play Rate over time",
       caption = glue::glue("{params$patch}
                            Ranked games from {params$start} UTC to {params$end} UTC
                            Metadata of games collected with RiotGames API")
  ) +
...
ggsave(glue::glue("./images/meta-report/{params$ind}-playrate-daybyday-diamond.png")) |> suppressMessages()
```
:::
...
MUtblTop.Diamond <- tibble::tibble( 
  ...
  mutate( muWR = replace(muWR, muGames < 10, NA ) )
```
...
```{r lmi, fig.height=8, fig.width=12}
WR.DT |>
  # filter( nGames > 200 ) |>
  ...
  # scale_color_manual(breaks = c("Tier0", "Tier1", "Tier2"), values=c("red", "blue", "green"))
  theme_539() +
  geom_label_repel( 
    aes(label=tooltip),
    family = "Roboto",
...
```{r missingCards}
missingCards <- LoR.Card$cardCode[LoR.Card$name %!in% TableAllCards$card]
...
<!-- Special thanks to **** for the recent support ^^ [^4] -->
