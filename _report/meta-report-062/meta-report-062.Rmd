...
params:
  ind: "MR063"
  patch: "Patch 3.8 - Week 3"
  title: "THE META REPORT NAME IS TOO LONG, TOO DAMN LONG (n°63)"
  description: "LoR Twitter: bla bla game dead, bla bla Rito nerf this dec, bla bla meta stale"
  cardlurl: "https://dd.b.pvp.net/latest/set6/en_us/img/cards/06BW006-full.png"
  start: "2022-06-08 18:00:00" #UTC tz / 'current' week start
  end:   "2022-06-15 18:00:00" #UTC tz / 'current' week end
  nGrid: 10
  nDeck_cc:  20
  nDeck_day: 12
  min_games: 50
...
EU_master_ladder   <- lorR::lor_leaderboard("europe",names = T) |> length()
NA_master_ladder   <- lorR::lor_leaderboard("americas",names = T) |> length()
APAC_master_ladder <- lorR::lor_leaderboard("sea",names = T) |> length()
```
...
nGames_diamond  <- NROW(LoR_Melt_Master_RMD)
...
tbl_master  <- table_by_the_numbers(data = LoR_Match_DT_RMD)
tbl_diamond <- table_by_the_numbers(data = LoR_Diamond_DT_RMD)

max_time = max(max(LoR_Match_DT_RMD$game_start_time_utc,na.rm = T),max(LoR_Diamond_DT_RMD$game_start_time_utc,na.rm = T))
...
nGames_tbl <- 
  ...
  tab_footnote(
    footnote = md(glue::glue(
      "EU Master {EU_master_found}/{EU_master_ladder}
      NA Master {NA_master_found}/{NA_master_ladder}
      APAC Master {APAC_master_found}/{APAC_master_ladder}
      "
      )), 
