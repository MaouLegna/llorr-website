...
preview: "https://dd.b.pvp.net/latest/set5/en_us/img/cards/05BC188-full.png"
author:
...
draft: false
params:
  prev:  "2021-09-08 21:00:00" #UTC tz / 'current' previous week start
  start: "2021-09-15 21:00:00" #UTC tz / 'current' week start
  end:   "2021-09-22 21:00:00" #UTC tz / 'current' week end
...
metathis::meta() |>
  ...
  metathis::meta_social(
    title = "THE META REPORT NAME IS TOO LONG, TOO DAMN LONG (n°23)",
...
* `r kableExtra::text_spec("New:", color = "red")` RubinBait - BandleBurn - not make from Rubin but same logic
...
nGames_tbl <- tbl_merge( 
  ...
  tab_header(
    ...
    subtitle = "Patch 2.15 second week / Master players"
    #'**************************************************************************** *
...
LoR.Melt.Matches.RMD[deck_code %in% RubinBait_deckCode_3, player:="RubinBait - BandleBurn" ]
...
# Deck (builder) of the week - RubinZoo Edition {.tabset .tabset-fade .tabset-pills}

As I added a section dedicated to decks' information which includes [Meta and Underdog decks](www.llorr-stats.com/static/meta.html) this section can be dedicated to more obscure decks are peculiar cases. And this weeks it is impossible not to consider the new meta-impact from RubinZoo with his bait-decks.
...
::::: {.panelset}
::: {.panel}
## Sunfolk Invoke - by ImpetousPanda
...
```{r tweet1}
tweetrmd::include_tweet("https://twitter.com/ImpetuousPanda/status/1439945808900669445?s=09",plain = TRUE)
```
```{r before-ad}
deck.of.week <- "Leona / Zoe (BC/MT)"

# number of games Before Tweet
nGames.BT <- LoR.Melt.Matches.RMD |>
  filter(player == deck.of.week ) |>
  filter( game_start_time_utc <= as.POSIXct("2021-09-20 15:33:00", tz = "UTC") ) |>
  count()

# after tweet
nGames.AT <- LoR.Melt.Matches.RMD |>
  filter(player == deck.of.week ) |>
  filter( game_start_time_utc > as.POSIXct("2021-09-20 15:33:00", tz = "UTC") ) |>
  count()
```

He posted this tweet at 2021-09-20 15:33, how many games were played during this week but before the tweet? not many, just `r nGames.BT`.

What is the number AFTER the tweet? `r nGames.AT`
...
DT.Aggreation <- add_column(DT.Aggreation,size = DT.Aggreation$LMI^2)
```

```{r ggplotly-LMI,echo=FALSE, fig.width=12, fig.height=8}
textWRPR <- function(LMI,Deck,WR, playrate){
...
fig <- DT.Aggreation %>%
  ...
  mutate( tooltip = textWRPR(LMI,player,WR,playrate) ) |>
  rename("Deck"="player","Win_Rate"="WR","Play_Rate"="playrate","WR Index"="wr_ind","Freq Index"="freq_ind","Tier"="tier") |>
  plot_ly(
    type = 'scatter',
    mode = 'markers',
    x = ~`WR Index`,
