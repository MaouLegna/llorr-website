...
params:
  ind: "MR039"
  patch: "Patch 2.22 - Week 4"
  title: "THE META REPORT NAME IS TOO LONG, TOO DAMN LONG (n°39)"
  description: "Twas the Day of the Balance Patch"
  cardlurl: "https://dd.b.pvp.net/latest/set5/en_us/img/cards/05BC088T2-full.png"
  prev:  "2021-11-22 18:00:00" #UTC tz / 'current' previous week start
  start: "2021-12-29 18:00:00" #UTC tz / 'current' week start
  end:   "2022-01-05 18:00:00" #UTC tz / 'current' week end
  nGrid: 10
...
masterapac <- fread(file.path("C:", "LlorR", "data", "raw", "account", "LoR_Master_S11_AS.csv"), header = T, na.strings = c("", NA), encoding = "UTF-8") |>
  filter(time <= as.POSIXct(params$end, "UTC"))
...
```{r plot-region-freq, fig.width=12, fig.height=8}
plot_freq(
  Region.Freq,
  nGames.Master,
...
# cowplot::save_plot(filename = glue::glue("images/{params$ind}-reg-freq.png"),plot = Reg.plot_freq, base_width = 12, base_height = 8)
```
...
```{r plot-region-freq-bycards, fig.width=12, fig.height=8}
plot_freq(
  Region.Freq.byCards,
  nGames.Master,
...
# cowplot::save_plot(filename = glue::glue("images/{params$ind}-reg-freq-bycards.png"),plot = Reg.plot_freq_bycards, base_width = 12, base_height = 8)
```
:::
...
ChampionCombo |>
  ...
  annotate(geom = "text", x = 3, y = 0.05, col = "black", label = paste(
...
ggsave(glue::glue("images/{params$ind}-topPlayrate.png"), width = 12, height = 8) |> suppressMessages()
```
:::
...
WR.DT |>
  ...
  reactablefmtr::save_reactable(glue::glue("images/{params$ind}-loritalia-playrate.png"))
```
...
gtsave(gtWinning, glue::glue("images/{params$ind}-topWin.png")) |> invisible()
```
...
Special thanks to bA1ance for the recent support (ᐛ)ᕗ
