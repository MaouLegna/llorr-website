...
WR.DT <- LoR.Melt.Seasonal |>
  ...
  filter( !(server == "apac" & game_start_time_utc %within% interval(with_tz(force_tz(timeBlock.5.start, "Japan") + minutes(60), "UTC"),
                                                                   with_tz(force_tz(timeBlock.5.end, "Japan") + minutes(60), "UTC"))) ) |>
  group_by(playerDeck) |>
  summarise(
...
Game.Result.APAC <-
  LoR.Melt.Seasonal |>
  filter(server == "apac") |>
  filter( !(game_start_time_utc %within% interval(with_tz(force_tz(timeBlock.5.start, "Japan") + minutes(60), "UTC"),with_tz(force_tz(timeBlock.5.end, "Japan") + minutes(60), "UTC"))) ) |>
...
ggtop32.APAC <- top32tbl.CC |>
  ...
  ggimage::geom_image(aes(x = name, image = img), y = 0, size = 0.040, inherit.aes = FALSE) +
  coord_flip() +
