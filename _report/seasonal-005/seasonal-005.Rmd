...
params:
  ind: "S2022-05-14"
  title: "THE SEASONAL REPORT IS VAST (n°5)"
  description: "Seasonal Tournament - A Curious Journey - Open Rounds"
  cardlurl: "https://dd.b.pvp.net/latest/set1/en_us/img/cards/01DE001-full.png"
  seasonalDate: "2022-05-14"
  final: FALSE
  patch: "live-green-3-07-13"
...
# Seasonal
LoR.Seasonal.DT.RMD     <- list.files(path = file.path("C:", "LlorR", "data", "raw"), pattern = glue("^(.*)Seasonal_DT(.*)csv$"), full.names = T) |>
  str_subset(pattern = "Keys",negate = T) |>
  map_dfr(data.table::fread,header = T, na.strings = c("", NA), encoding = "UTF-8") |>
  mutate(server = if_else(server %in% c("sea","asia"),"apac",server) )
...
LoR.Melt.Seasonal <- 
  LoR.Seasonal.DT.RMD |>
  filter(game_mode == "SeasonalTournamentLobby" & game_version == params$patch) |>
  ...
  left_join(
    LoR.Account.RMD |>
      select(playerPuuid = puuid, playergameName = gameName, playerID = RiotID),
    by = "playerPuuid"
  ) |>
  left_join(
    LoR.Account.RMD |>
      select(opponentPuuid = puuid, opponentgameName = gameName, opponentID = RiotID),
    by = "opponentPuuid"
  )
```
...
LoR.Melt.Matches.RMD <- LoR.Match.DT.RMD |>
  ...
  filter(game_start_time_utc %within% interval(
    with_tz(force_tz(timeBlock.1.start, "Asia/Shanghai"), "UTC") - lubridate::days(7),
    with_tz(force_tz(timeBlock.1.start, "Asia/Shanghai"), "UTC")
  )) |>
  rename(playerPuuid_1 = puuid_1, playerPuuid_2 = puuid_2) |>
  mutate(opponentPuuid_1 = playerPuuid_2, opponentPuuid_2 = playerPuuid_1) |>
  mutate(opponent_deck_code_1 = deck_code_2, opponent_deck_code_2 = deck_code_1) |>
...
```{r check-value}
# LoR.Melt.Seasonal[is.na(playerPuuid), .N]
...
Number of (Seasonal) matches analyzed **`r format(NROW(LoR.Melt.Seasonal)/2,scientific = F)`** or **`r format(NROW(LoR.Melt.Seasonal),scientific = F)`** games.
...
Game.Result.APAC <-
  LoR.Melt.Seasonal |>
  filter(server == "apac") |>
  filter( !(game_start_time_utc %within% interval(with_tz(force_tz(timeBlock.5.start, "Asia/Shanghai"), "UTC"),with_tz(force_tz(timeBlock.5.end, "Asia/Shanghai"), "UTC"))) ) |>
  ...
  mutate(round = case_when(
    timeBlock > with_tz(force_tz(timeBlock.1.start, "Asia/Shanghai"), "UTC") & timeBlock <= with_tz(force_tz(timeBlock.1.end, "Asia/Shanghai"), "UTC") ~ 1, # 04:00
    timeBlock > with_tz(force_tz(timeBlock.2.start, "Asia/Shanghai") + minutes(150), "UTC") & timeBlock <= with_tz(force_tz(timeBlock.2.end, "Asia/Shanghai") + minutes(150), "UTC") ~ 2, # 07:35
    timeBlock > with_tz(force_tz(timeBlock.3.start, "Asia/Shanghai") + minutes(150), "UTC") & timeBlock <= with_tz(force_tz(timeBlock.3.end, "Asia/Shanghai") + minutes(150), "UTC") ~ 3, # 08:40
    timeBlock > with_tz(force_tz(timeBlock.4.start, "Asia/Shanghai") + minutes(150), "UTC") & timeBlock <= with_tz(force_tz(timeBlock.4.end, "Asia/Shanghai") + minutes(150), "UTC") ~ 4, # 09:45
    timeBlock > with_tz(force_tz(timeBlock.5.start, "Asia/Shanghai") + minutes(150), "UTC") & timeBlock <= with_tz(force_tz(timeBlock.5.end, "Asia/Shanghai") + minutes(150), "UTC") ~ 5, # 10:50
    
    timeBlock > with_tz(force_tz(timeBlock.6.start, "Asia/Shanghai") + minutes(125), "UTC") & timeBlock <= with_tz(force_tz(timeBlock.6.end, "Asia/Shanghai") + minutes(125), "UTC") ~ 6, # 11:55
    timeBlock > with_tz(force_tz(timeBlock.7.start, "Asia/Shanghai") + minutes(125), "UTC") & timeBlock <= with_tz(force_tz(timeBlock.7.end, "Asia/Shanghai") + minutes(125), "UTC") ~ 7, # 13:00
    
    timeBlock > with_tz(force_tz(timeBlock.8.start, "Asia/Shanghai") + minutes(125), "UTC") & timeBlock <= with_tz(force_tz(timeBlock.8.end, "Asia/Shanghai") + minutes(125), "UTC") ~ 8, # 14:05
    timeBlock > with_tz(force_tz(timeBlock.9.start, "Asia/Shanghai") + minutes(125), "UTC") ~ 9 # 15:10
  )) |>
...
# WR tbl
seasonal_WR_DT <- win_rates(LoR.Melt.Seasonal,server = F)
...
```{r compute-MU-ladder}
# WR tbl
ladder_WR_DT <- win_rates(LoR.Melt.Matches.RMD,server = F)

# MU tbl the date is already filtered
ladder_MUtbl <- LoR.Melt.Matches.RMD |>
  match_ups() |>
  mutate(
    direction = replace(direction,direction==0.50,"TIE"),
    mirror = if_else(playerDeck==opponentDeck,"Yes","No" )
...
CC.table.S |>
  ...
  labs(
    ...
    caption = element_text(
      glue::glue("Playrates from {(with_tz(force_tz(timeBlock.1.start,'Asia/Shanghai'),'UTC')-lubridate::days(7))} to {with_tz(force_tz(timeBlock.1.start,'Asia/Shanghai'),'UTC')}, one week before the start of the earliest Seasonal start (the Apac shard)
