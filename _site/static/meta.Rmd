---
title: "Meta & Decks (POST-HOTFIX from Patch 2.21)"
author: "by Legna"
...
params:
  ...
  end:     "2022-01-01 18:00:00" #UTC tz /
---
...
LoR.Melt.Matches.RMD <- LoR.Match.DT.RMD |>
  filter( game_type=="Ranked" ) |>
  filter( game_start_time_utc > as.POSIXct(params$hotfix, tz = "UTC") ) |>
...
```{r value-games}
flexdashboard::valueBox(glue::glue("{LoR.Melt.Matches.RMD[ ,.N ]} Games"),
...
LoR.Melt.Matches.RMD <- LoR.Match.DT.RMD |>
  ...
  filter( game_type=="Ranked" ) |>
  filter( game_start_time_utc > as.POSIXct(params$start, tz = "UTC") ) |>
  select( match_key, game_start_time_utc, starts_with("deck_code") ) |>
  left_join(LoR.Deck.RMD[,.(deck_code,playerDeck_1=archetype)],  by=c("deck_code_1"="deck_code")) |>
  left_join(LoR.Deck.RMD[,.(deck_code,opponentDeck_1=archetype)],by=c("deck_code_2"="deck_code")) |>
...
#'#########################################################
LoR.Melt.Diamond.RMD <- LoR.Match.DT.RMD |>
...
meta_evo.base <- Champion.deck.daily |>
  ...
  labs(x = "Date",
       y = "PlayRate",
       title = "(Master Players) Evolution of Play Rate over time"
...
meta_evo.diamond.base <- Champion.deck.daily.diamond |>
  ...
  labs(x = "Date",
       y = "PlayRate",
       title = "(~HighDiamond) Evolution of Play Rate over time",
       caption = glue::glue("{params$patch}
                            Ranked games from {params$start} UTC to {params$end} UTC
...
```{r value-player-base}
flexdashboard::valueBox(glue::glue("{LoR.Account.RMD[master=='master',.N]}"),
...
flexdashboard::valueBox(glue::glue("{LoR.Account.RMD[master=='diamond',.N]} ~HighDiamond Players"),
