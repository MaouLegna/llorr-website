...
output:
  flexdashboard::flex_dashboard:
    navbar:
      - { icon: "fa-trophy", text: "MatchUps", href: "https://www.llorr-stats.com/static/meta.html", align: right }
...
Champion.deck.daily.byserver <- LoR.Melt.Matches.RMD |>
  ...
  .[ , playRate := (cumsum(num_cc)/num_cc)/.I , by=Archetype ] |>
  select(!num_cc) |>
...
Champion.deck.daily.byserver <- Champion.deck.daily.byserver |>
  ...
  group_by(game_start_time_utc,server,Archetype) |>
  slice_head(n=1) |>
  ungroup() |>
  group_by(server,Archetype) |>
...
crosstalk::filter_select(id = "player", label = "Archetype", sharedData = shared_code, group = ~player)
```
