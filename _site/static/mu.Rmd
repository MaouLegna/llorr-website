---
title: "Match Ups Data (POST-HOTFIX from Patch 2.21)"
author: "by Legna"
output:
...
params:
  ...
  start:   "2021-12-14 18:00:00" #UTC tz / 2.21 Hotfix
...
#' WR tbl
#'#######
WR.DT.diamond <- LoR.Melt.Diamond.RMD |>
  filter(game_outcome!="tie") |>
  select( playerDeck,opponentDeck,game_outcome ) |>
  group_by(playerDeck) |>
  summarise(
...
flexdashboard::valueBox(glue::glue("{nGames.diamond} Ranked Games"),
                        caption = glue::glue("out of a total of {NROW(LoR.Melt.Diamond.RMD)} - min 10 games"),
                        icon = "fa-table",
                        color = "#FA7404")
```
...
grid.plus <- matrix(MUtbl.Diamond$muWR,
                    nrow = params$nGrid,
                    ncol = params$nGrid, byrow = T)
...
MUgrid.gt <- as.data.table(cbind(grid,grid.plus)) |>
...
```{r grid-diamond}
# grid.diamond <- matrix(MUtbl.Diamond$muWR,
#                        nrow = params$nGrid,
#                        ncol = params$nGrid, byrow = T)
# 
# diag(grid.diamond) <- NA
# colnames(grid.diamond) <- TopCC
# 
# grid.CI <- matrix(MUtbl.Diamond$CI,
#                     nrow = params$nGrid,
#                     ncol = params$nGrid, byrow = T)
# 
# diag(grid.CI) <- NA
# colnames(grid.CI) <- sprintf("%s.v2",TopCC)
# 
# grid.CI <- grid.CI |>
#   as.data.table()
# 
# grid.diamond.all <- cbind(grid.diamond,grid.CI)
# 
# MUgrid.gt.Diamond <- grid.diamond.all |>
#   as.data.table() |>
#   add_column( name = colnames(grid.diamond),.before = 1 ) |>
#   rename(" "=1) |>
#   gt() |>
#   gtExtras::gt_color_rows(2:(params$nGrid+1),
#                 # palette = c("#D73027", "white", "#1A9850" ),
...
#   #   gtExtras::gt_merge_stack(col1 = !!TopCC[i], col2 = !!sprintf("%s.v2",TopCC[i]) )
#   
#   MUgrid.gt.Diamond <- MUgrid.gt.Diamond |>
#     cols_merge(
#       columns = c(i+1,params$nGrid+i+1),
#       hide_columns = (params$nGrid+i+1),
#       # pattern = "{1}&mdash;{2}"
...
#         glue::glue(
#           #595959 #grey35 / #7f7f7f #grey50
#           "<div style='line-height:12px'><span style='font-weight:bold;color:#595959;font-variant:small-caps;font-size:14px'>{upper}</div>
#            <div style='line-height:10px'><span style='color:#7f7f7f;font-size:10px'>{bottom}</span></div>"
#         )
#         }
#       )
#   
...
#   ) |>
#   tab_source_note(
#   source_note = md(glue::glue("MatchUp values from Ranked games with ~HighDiamond Ranked players.
#                               Order of the Archetypes based on the playrate.
#                               Source: Metadata of games collected with RiotGames API")
#   ))
```
...
MUtbl.diamond |>
  select(playerDeck,opponentDeck,muWin,muGames,muWR) |>
  downloadthis::download_this(
    button_label = "Download MU (~HighDiamond) data as .csv",
    output_name = "mutable",
    button_type = "success",
    has_icon = TRUE,
    icon = "fa fa-save"
