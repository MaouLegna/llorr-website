---
title: "Match Ups Data (from Patch 3.8 WorldWalker)"
author: "by Legna"
...
flexdashboard::valueBox(glue::glue("{nGames_diamond} Ranked Games"), 
                        caption = glue::glue("out of a total of {NROW(LoR_Melt_Diamond_RMD)} - min {params$min_mu_games} games"), 
                        icon = "fa-table", 
                        color = "#FA7404")
```
...
crosstalk::filter_slider("playrate", "Player PlayRate",     crosstalk_diamond, ~playrate, step=0.01, min = 0, max = round(max(MU_tbl$playrate), 4) )
crosstalk::filter_slider("opponentPR", "Opponent PlayRate", crosstalk_diamond, ~opponentPR, step=0.01, min = 0, max = round(max(MU_tbl$opponentPR), 4) )
...
```{r}
flexdashboard::valueBox(glue::glue("{nGames} Ranked Games - Master Players"), 
...
gtsave(data = gtMU_grid, glue::glue("images/{grid_name}.png"), vwidth = 1500, vheight = 1000) |> invisible()
...
```{r}
grid_source_note <- md(glue::glue("MatchUp values from Ranked games of the current HighDiamond rank players
                                  players who played against Master players but are not Master themselves
                                  Order of the Archetypes based on the playrate.
                                  Source: Metadata of games collected with RiotGames API") )
...
gtsave(data = gtMU_grid, glue::glue("images/{grid_name}_diamond.png"), vwidth = 1500, vheight = 1000) |> invisible()
```
...
#   # filter(game_start_time_utc %within% interval(as.POSIXct(params$start,"UTC"), # +days(7),
#   #                                              as.POSIXct(params$end,"UTC") ) ) |>
...
#   opponentDeck = rep(TopCC,times=30)
#   ) |>
#   left_join( MUtbl, by = c("playerDeck","opponentDeck")) |>
#   # mutate( muWR = replace(muWR, muGames < params$minMUGames, NA ) ) |>
#   fwrite(file.path("C:","LlorR","scripts","shiny","bo3-helper","Bandle03-MUtbl.csv"))
```
