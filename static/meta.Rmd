---
title: "Meta & Decks (from Patch 3.6 - Late April Patch)"
author: "by Legna"
...
seqDate <- seq(as.POSIXct(params$start, tz = "UTC"),ceiling_date(lubridate::now(),unit = "day")+hours(18) ,"day") #

one_day_check <- length(seqDate)>1
if (one_day_check) seqDate <- seqDate[-length(seqDate)]
```
...
top7days <- LoR.Melt.Master.RMD |>
  ...
  slice_max(n,n=params$ntopPR,with_ties = F) |>
  pull(playerDeck)
...
max_freq <- max(c(Champion.deck.daily$freq,Champion.deck.daily.diamond$freq))
```
...
meta.evo.base <- Champion.deck.daily |>
  ...
  scale_y_continuous(
    ...
    limits = c(0,params$meta_max),
    breaks = seq(0,params$meta_max,0.05),
    labels = scales::percent_format(accuracy = 1) # glue::glue("{format(seq(0, 0.20, by = 0.05), nsmall = 2)}%")
  )  +
  labs(x = "Date",
       y = "Daily PlayRate",
       title = "Master PlayRates",
       subtitle = "Evolution of Play Rate over time"
