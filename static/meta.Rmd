...
#' Read Temp Games
##################
LoR.S11.RMD <- fread(file.path("C:","LlorR","data","raw","LoR_S11DT.csv"), header = T, na.strings = c("",NA))

LoR.S11.RMD <- LoR.S11.RMD |>
  filter(!is.na(participants_1))

LoR.S11.DT <- fread(file.path("C:","LlorR","data","raw","LoR_S11DT.csv"),
                    colClasses = "character", header = T, na.strings = c("", NA)) |>
  # mutate(across(where(is.character), ~na_if(., ""))) |>
  mutate(across(c(starts_with('order'),status), as.numeric  ) )
  

LoR.S11.DT <- LoR.S11.DT |>
  select(-ends_with("_3"),-ends_with("_4"))
  
sapply(LoR.S11.DT,class) |> unname()
...
seqDate <- seq(as.POSIXct(params$start, tz = "UTC"),floor_date(lubridate::now(),unit = "day")-days(1)+hours(18) ,"day") #
```
