...
WR.DT.d7 <- LoR.Melt.Matches.RMD |>
  filter( game_start_time_utc >= Sys.Date()+hours(17)-days(7) ) |>
  filter(game_outcome!="tie") |>
  select(playerDeck,opponentDeck,game_outcome ) |>
  group_by(playerDeck) |>
  summarise( 
...
LoR.Melt.24hrs <- LoR.Melt.Matches.RMD |>
  filter( 
      game_start_time_utc >= Sys.Date()+hours(17)-days(2) &
      game_start_time_utc <  Sys.Date()+hours(17)-days(1)
      )
...
flexdashboard::valueBox(glue::glue("24 Hours Time Frame"),
                        caption = glue::glue("from {Sys.Date()+hours(17)-days(2)} to {Sys.Date()+hours(17)-days(1)}"),
                        icon = "fa-clock",
                        # color = "lightgreen"
                        color = "red")
```
