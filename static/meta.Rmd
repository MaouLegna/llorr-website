---
title: "Meta & Decks (from Patch 2.21 onward)"
author: "by Legna"
...
#' Read Diamond DT
##################
LoR.Diamond.RMD  <- fread(file.path("C:","LlorR","data","raw","LoR_DiamondDT.csv"), header = T, na.strings = c("",NA))
...
deck_filter <- rbind(LoR.Match.RMD,LoR.Diamond.RMD) |>
  #' Base filters
...
#'#########################################################
LoR.Melt.Diamond.RMD <- LoR.Diamond.RMD |>
  filter( game_type=="Ranked" ) |>
  filter( game_start_time_utc > as.POSIXct(params$start, tz = "UTC") ) |>
  left_join(LoR.Deck.RMD[,.(deck_code,playerDeck_1=archetype)],  by=c("deck_code_1"="deck_code")) |>
  left_join(LoR.Deck.RMD[,.(deck_code,opponentDeck_1=archetype)],by=c("deck_code_2"="deck_code")) |>
...
meta_evo.base <- Champion.deck.daily |>
  ...
  labs(x = "Date",
       y = "PlayRate",
       title = "(Seasonal Players) Evolution of Play Rate over time"
...
meta_evo.diamond.base +
  ...
  plot_annotation(caption = glue::glue('Days starts at 18:00:00 UTC time of X day to 18:00:00 UTC time of X+1 day. During solar-time 18:00 UTC seems to be the release time of LoR content. Any detected changes will be reported.
                                       Reference Population; Seasonal players from Between Worlds Seasonal and High Diamond defined Games from Players who played against Master but aren not Master.
                                       Source: Metadata of games collected with RiotGames API'))
...
```{r value-player-base}
flexdashboard::valueBox(glue::glue("{LoR.Account.RMD[note=='seasonal',.N]} (prev) Seasonal Players"),
                        caption = glue::glue("Players identified in the Between Worlds Seasonal Tournament"),
                        icon = "fa-medal",
                        color = "#65c0a9")
```
