...
LoR.Melt.Matches.RMD <- LoR.Match.RMD |>
  ...
  #' finish 'process' data
  left_join(LoR.Account.RMD[,.(puuid,playerDeckID=RiotID)]   ,by=c("playerPuuid"="puuid")) |>
  left_join(LoR.Account.RMD[,.(puuid,opponentDeckID=RiotID)] ,by=c("opponentPuuid"="puuid")) |>
...
```{r filters-meta-evo}
crosstalk::filter_select("playerDeck", "Deck's Archetype", keyDeckDaily, ~playerDeck)
```
...
meta_evo <- keyDeckDaily %>% {
   ggplot(., aes(game_start_time_utc, playRate, group = playerDeck,color = playerDeck)) +
      geom_line() +
...
dot_plot <- keyDeckDaily |>
  ...
  add_markers(
    x = ~round(playRate,3),
    y = ~forcats::fct_reorder(playerDeck, playRate),
    hoverinfo = "x+y"
  ) %>%
  layout(
