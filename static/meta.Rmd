...
WR.DT.d7 <- LoR.Melt.Matches.RMD |>
  filter( game_start_time_utc >= Sys.Date()+hours(21)-days(7) ) |>
  filter(game_outcome!="tie") |>
  select(playerDeck,opponentDeck,game_outcome ) |>
  group_by(playerDeck) |>
  summarise( 
...
LoR.Melt.24hrs <- LoR.Melt.Matches.RMD |>
  filter( 
      game_start_time_utc >= Sys.Date()+hours(21)-days(2) &
      game_start_time_utc <  Sys.Date()+hours(21)-days(1)
      ) |>
  filter(game_outcome!="tie") |>
  # select(playerDeck,opponentDeck,game_outcome ) |>
...
deckCodeTbl.24hrs  <- LoR.Melt.Matches.RMD |>
  filter( game_start_time_utc >= Sys.Date()+hours(21)-days(1) ) |>
  filter( playerDeck %in% c(LoR.Melt.24hrs$playerDeck) ) |>
  mutate( playerDeck = factor(playerDeck, ordered = TRUE,
                              levels = c(LoR.Melt.24hrs$playerDeck) )
          ) |>
  group_by(server,playerDeck,deck_code ) |>
  summarise( n = n() ) |>
  slice_max(n, n = 10) |>
...
LoR.Melt.24hrs |>
  reactable(
    ...
    details = function(index) {
      # index = 1
      deckdata <- deckCodeTbl.24hrs[ deckCodeTbl.24hrs$playerDeck == LoR.Melt.24hrs$playerDeck[index], ]
      reactable(
        deckdata,
        fullWidth = F,
        defaultColDef = colDef(
...
flexdashboard::valueBox(glue::glue("24 Hours Time Frame"),
                        caption = glue::glue("from {Sys.Date()+hours(21)-days(2)} to {Sys.Date()+hours(21)-days(1)}"),
                        icon = "fa-clock",
                        # color = "lightgreen"
                        color = "red")
```
...
meta_evo.base + geom_label_repel(
    ...
    xlim = c(as.Date(Sys.Date()), as.Date(Sys.Date()+days(3)) )
  )
