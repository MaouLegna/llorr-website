...
```{r print-date-by-date, fig.width=12, fig.height=8}
meta_evo <- Champion.deck.daily |>
  ...
  geom_label_repel(
    ...
    # aes(label=str_to_upper(playerDeck),fill = unique(Champion.deck.daily$playerDeck) ),
    aes(label=str_to_upper(playerDeck),color = playerDeck ),
    # aes(label=str_to_upper(playerDeck)),
    # color = "white",
...
deckStrctures_meta  <- map_df(.x = c(fct_meta), ~get_deck_structure_2_meta(DT = LoR.Melt.Matches.RMD, archetype =.x, minN = 10) ) |>
  as_tibble() |>
  add_column(popularity = "Meta")

deckStrctures_underdog <- map_df(.x = c(fct_underdog), ~get_deck_structure_2_meta(DT = LoR.Melt.Matches.RMD,archetype =.x,minN = 10) ) |>
  as_tibble() |>
...
* The graph in the Meta-Evolution section displays the PlayRate over time of the top15 most plyayed decks in the last 7 days over all the timeframe considered.

* The PlayRate is computed for each
