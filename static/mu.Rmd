...
# MUtbl |>
#   filter( player %in% c("Ezreal / Karma","Miss Fortune / Poppy (BW/DE)") & opponent %in% c("Ezreal / Karma","Miss Fortune / Poppy (BW/DE)")  )
```

```{r}
# archetypes <- LoR.Melt.Matches.RMD |>
#   tabyl(player) |>
...
#   filter( player == archetypes[1] ) |>
#   # filter( player == "Maokai / Nautilus" ) |>
#   pull(deck_code) %>%
#   get_deck_structure_2(.,minN = 5)
```
