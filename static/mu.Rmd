...
MU_tbl |>
  filter(!is.na(playerDeck)) |>
  select(playerDeck,opponentDeck,muWR) |>
  pivot_wider(id_cols = playerDeck,names_from = opponentDeck,values_from = muWR) |>
  data.table::fwrite( file.path("C:","Users","Valentino Vazzoler","Documents","R","llorr-website","data","Grid-MUtbl.csv") )
```
...
MU_tbl |>
  ...
  mutate(muWR = round(muWR,digits = 3)) |>
