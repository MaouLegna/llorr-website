---
title: "Match Ups Data (from Patch 3.19 - Pre-World Patch)"
author: "by Legna"
...
params:
  ...
  min_mu_games: 50   # min number of games for a MU
  dim_grid: 15       # how many MU to plot
  last_season: "S16"
  grid_name: "mugrid_319"
  set_names: "Set06-04"
---
...
LoR_Temp_DT_RMD <- list.files(path = file.path("C:", "LlorR", "data", "raw"), pattern = glue("^(.*)Temp_DT_{params$last_season}(.*)csv$"), full.names = T) |>
  str_subset(pattern = "Keys", negate = T) |>
  map_dfr(data.table::fread, header = T, na.strings = c("", NA), encoding = "UTF-8") |>
  mutate(server = if_else(server %in% c("sea", "asia"), "apac", server) )
```
...
Shiny_MUtbl <- LoR_Melt_Match_RMD |>
  ...
  filter(playerDeck %in% top_archetype$playerDeck & opponentDeck %in% top_archetype$playerDeck) |>
  match_ups() |>
  left_join(top_archetype) |>
  rename(LCI = lower, UCI = upper)

shiny_grid <- mu_grid(mu_data = Shiny_MUtbl, dim_grid = 30, 
                      games_data = LoR_Melt_Match_RMD, prev_days = 7,
                      grid_source_note = "", div_1 = 10, div_2 = 10, font_upper = 12, font_lower = 9)

Shiny_MUtbl |> data.table::fwrite( file.path("C:","LlorR","scripts","shiny","bo3-helper",glue::glue("{params$set_names}-MUtbl.csv") ) )
shiny_grid |>  gt::gtsave( file.path("C:","LlorR","scripts","shiny","bo3-helper",glue::glue("{params$grid_name}.png") ) , vwidth = 1500, vheight = 1000) |> invisible()

# Shiny_MUtbl <- data.table::fread(file.path("C:","LlorR","scripts","shiny","bo3-helper",glue::glue("{params$set_names}-MUtbl.csv")), header = T, na.strings = c("", NA), encoding = "UTF-8")

Shiny_MUtbl |>
  select(playerDeck,opponentDeck,muWR) |>
  pivot_wider(id_cols = playerDeck,names_from = opponentDeck,values_from = muWR) |>
  data.table::fwrite( "../data/Grid-MUtbl2.csv" )
```
...
grid_source_note <- md(glue::glue("MatchUp values from Ranked games of the current HighDiamond rank players players who played against Master players but are not Master themselves
                                  Order of the Archetypes based on the playrate.
                                  WR<sub>meta</sub> defined as the win-rate against decks with at least 1% playrate.
                                  Source: Metadata of games collected with RiotGames API") )
...
### Patch 3.14 - Patch 3.15 - Darkin Saga: Awekening

![](images/mugrid_314_315.png)
