...
LoR.Match.RMD |>
  mutate(time = ymd_hms(game_start_time_utc) ) |>
  filter(time <= with_tz( force_tz( ymd_hms("2021-03-03 10:00:00"),"US/Pacific") ,"UTC") ) |>
  select(-ends_with("_3"),-ends_with("_4")) |>
  select(-time) -> S00

# LoR.Match.RMD |>
#   filter(match_key %!in% S00$match_key ) -> LoR.Match.RMD

LoR.Match.RMD |>
  mutate( row = row_number() ) |>
  mutate(time = ymd_hms(game_start_time_utc) ) |>
  filter(time >= with_tz( force_tz( ymd_hms("2021-06-30 10:00:00"),"US/Pacific") ,"UTC") ) |>
  slice_min(row,n=1) |>
  select(row,time)
...
LoR.Match.RMD |>
  mutate(row=row_number() ) |>
  mutate(time=ymd_hms(game_start_time_utc) ) |>
  # select(game_start_time_utc,time)
  slice(1908227:n()) |>
  filter(is.na(time) | time >= with_tz( force_tz( ymd_hms("2021-06-30 10:00:00"),"US/Pacific") ,"UTC") ) -> S8
  # tabyl(game_version)

# S8 |>
#   select(-row,-time) |>
#   select(-ends_with("_3"),-ends_with("_4")) |>
#   fwrite("LoR_MatchDT_S08_211_213.csv")

# LoR.Match.RMD |>
#   filter(match_key %!in% S8$match_key)

LoR.Match.RMD |>
  mutate( row = row_number() ) |>
  mutate(time = ymd_hms(game_start_time_utc) ) |>
  filter(time >= with_tz( force_tz( ymd_hms("2021-05-05 10:00:00"),"US/Pacific") ,"UTC") ) |>
  slice_min(row,n=1) |>
  select(row,time)
...
LoR.Match.RMD |>
  ...
  # select(game_start_time_utc,time)
  slice(1009562:n()) |>
  # filter(is.na(time) | time >= with_tz( force_tz( ymd_hms("2021-05-05 10:00:00"),"US/Pacific") ,"UTC") )
  filter(is.na(time) | time %within% interval(
    with_tz( force_tz( ymd_hms("2021-05-05 10:00:00"),"US/Pacific") ,"UTC"),
    with_tz( force_tz( ymd_hms("2021-06-30 10:00:00"),"US/Pacific") ,"UTC"))
    ) -> S7

prova <- S7 |>
  pivot_longer(cols = c(ends_with("_1"), ends_with("_2"),
                        ends_with("_3"), ends_with("_4")),
               names_to = c(".value"), names_pattern = "(.*)_[0-9]") |>
  distinct() |>
  filter(!( status==200 & is.na(participants) ) )
