...
Let's start by informing how the amount of 'Akshan/Sivir/Zed' amoutnto `r ASZ`.
...
```
The overall prevalence of '3 champions deck' while it may looks higher than common believes (`r nChamp3.percent`) the value is mostly carried by the Akshan/Sivir/Zed which amoutn to  decks as they are about half the cases of 3 champs decks `r round(ASZ/nChamp3,3)`%

Also worth noticing is how among the Sivir/Zed and Sivir/Akshan decks the '3 champ' option is choosen also about half of the times and the other half is indeed mostly Akshan/Sivir/Zed decks `r round(ASZ/SZ3,3)`% when we consider Sivir/Zed decks and `r round(ASZ/SA3,3)`% when we consider Sivir/Akshan decks.
...
grViz("
      ...
      # add node statements
      a -> {b,c} [color = green, label = 'subset']
      b -> a [color = red]
      c -> a [color = red]
      ...
      ")
```
* The first step would be finding whenever Sivir/Zed (SZ) and Sivir/Akshan (SA) decks are a specific case of Akshan/Sivir/Zed (ASZ) decks.
