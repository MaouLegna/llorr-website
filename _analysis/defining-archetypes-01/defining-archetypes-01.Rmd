...
While I presented a method for comparing archetypes this is just one method among a myriad of alternatives. What I would like to try is trying to solve the problem by applying graph theory.

When looking at an archetype it's in fact possible to see it as a newtork between nodes as the cards and the edges the information related to how often some cards are played toghether. The advantage would be having to compare a single item for each archetype, the archetype-graph but with the disadvantage of working with a more complex structure which is the reason it's not applied in this paper.
