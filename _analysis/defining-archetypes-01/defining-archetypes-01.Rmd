...
meta() %>%
  ...
  meta_social(
    ...
    image = "./images/archetypes/01-ASZSZ.png",
    image_alt = "ASZSZ",
    og_type = "website",
    og_author = "Legna",
...
ggsave("./images/archetypes/01-ASZAS.png")
```
```{r ASZ-SZ}
deck.matrix <- LoR.Card$cardCode %>% purrr::map_dfc(setNames, object = list(numeric()))
...
```{r plot-ASZ-SZ, fig.cap="distribution of cosine distances for ASZ vs SZ decks"}
DSim.tbl_2 |>
  ggplot(aes(x = value, fill=group)) +
  geom_density() +
  theme(legend.position = "bottom") +
...
ggsave("./images/archetypes/01-ASZSZ.png")
```
...
```{r pair-wise}
# attach(DSim.tbl_2)
# print(pairwise.t.test(value, group, p.adj = "bonf",))
# detach()
# 
# attach(DSim.tbl_2)
# print(pairwise.t.test(value, group, p.adj = "holm",))
# detach()
```
```{r ecdf, fig.cap="ecdf of cosine distances between the deckcodes that accounts for at least 50% of the games played", fig.width=8}
# DSim.tbl_1 |>
...
#   labs( x = "x", y = "Cumulative probability" ) +
#   
# plot_annotation(title = 'Comparison of Empirical Cumulative Distribution Functions',
#                 caption = 'ecdf of cosine distances between the deckcodes that accounts for at least 50% of the games played') +
#   plot_layout(guides = 'auto', ncol = 2)
```
