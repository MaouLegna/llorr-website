...
LoR.Melt.Games.RMD |> 
  ...
  geom_segment(aes( x=20, y=gnar_mean_wr+0.2,xend=20,yend=gnar_mean_wr ),arrow = arrow( length = unit(0.03,"npc") ) ) +
  geom_label(aes(x=20,y=gnar_mean_wr+0.2,alpha="semitransparent"),label="Mean Win Rate \n of Gnar-Timelines",size=3.5,fill="#FDCE2A" ) +
  labs(x="Total Games",
       y="Win Rate",
       title="Cavemen Time - Mastery Curve",
       subtitle = "Mastery Curve on Gnar Timelines decks",
       caption = glue("Constructed games from Seasonal Players (n={length(puuid_seasonal)})
                      The data under the 5th percentile and over the 90th percentile are removed
                      The estimated Mastery-Curve is a linear model with up to the third polynomial") ) +
  theme_539() +
  scale_x_continuous(n.breaks = 10) +
  scale_y_continuous(
    n.breaks = 10,
  ...
  theme(legend.position = "none") -> plot_cavemen_time
...
LoR.Melt.Games.RMD |> 
  ...
  group_by(nGames) |>
  summarise(meanWR = mean(meanWR)) |>
  filter( between(nGames, quantile(nGames,0.05,names = F), quantile(nGames,0.90,names = F) ) ) -> data

model <- lm(meanWR~nGames+I(nGames^2)+I(nGames^3),data = data)
summary(model)
