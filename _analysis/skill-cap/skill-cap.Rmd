...
```{r functions}
simulate_meta <- function(weight,rank,n) {
  
  W <- c(weight,1-weight) 
  P <- c(mu_wr1,mu_wr2)
  M <- W %*% t(P) |> round(digits = 3)
  
  # print(M)
  tibble(outcome = c( rep(1,M[1,1]*n),rep(0,M[1,2]*n),
                      rep(0,M[2,1]*n),rep(1,M[2,2]*n) ),
         mu = c( rep("MU1",sum(M[1,])*n) ,
                 rep("MU2",sum(M[2,])*n) ),
         rank = rank
  )
}
...
# }
# 
# lapply(seq(0.01,1,0.01), function(x) wr(x,.6) )

ranks_levels <- c("platinum","diamond","master")
Ranks_levels <- c("Platinum","Diamond","Master")
...
example <- pmap_dfr(.l = list(weights = c(w1_plat,w1_diamond,w1_master),
                              ranks = ranks_levels,
                              n = c(10000,5000,3000) ),
         .f = ~simulate_meta(..1,..2,..3) ) |>
  mutate(rank = factor(rank,levels = ranks_levels, labels = Ranks_levels,ordered = T) )
```
...
As the data is collected as different ranks platinum,diamond and master rank; while looking at these results the high ranked players may have some doubts about the results as they are sure that overall they winning more than losing.

The stratified win rate is obtained:
...
In this example the *meta* is defined by a simple case two different match ups (let's ignore if it's feasable or not)
...
Assuming it was possible to collect all information a game played it would be still hard to control for
...
> Hat: ...that's probably the player influence. If something is only good at top100, that mean that the best players in the world are playing a mediocre deck and off-setting for that Because the very best players in the world, will absolutely make anything look good
...
It's highly possible that Zach uses a more complex methodology compared to the one we will present as they have access to a bigger pool of variables but the core concept should still remain similar.
...
- Measuring the Skill-Cap by looking at the differences between Top Legends/Diamond I-IV

This is the key point which along side the previous one should explain the core methodology.

The unit of choice if the difference in win rates among match-ups.

It means that given j,k with $j \ne k$ ranks of players, for a certain deck we have $d_{j,k}(deck\_MU_i) = wr_j(deck\_MU_i)-wr_k(deck\_MU_i)$ with $i \in 1:n$ with n being all the shared match ups among rank j,k of the chosen decks.

With these an intuitive use of the set of differences if the take the expected value which would be $SkillCap(deck)_{j_k} = E(d_{j,k}(deck\_MU_i))$

- After removing all other factors and seeing the disparity in MU

This is mostly to remember that there is the chance that the methodology also control for other variables

---

Problems:

Assuming almost all MU are there in any combination rank and that 

---

But differently for the Skill-Curve they don't look at the WR in function of games but at each MU-WR disparity
