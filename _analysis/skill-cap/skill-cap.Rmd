...
In this article we try to explore the concept of Skill-Cap explained by

[][https://twitter.com/ZachODR]
...
```{r vS-example}
ranks_levels <- c("platinum","diamond","master","top master")

set.seed(123)
example <- tibble(
  outcome = c(
    sample(x = c(0,1),size = 1000,replace = T,prob = c(0.55,0.45)),
    sample(x = c(0,1),size = 1000,replace = T,prob = c(0.50,0.50)),
    sample(x = c(0,1),size = 1000,replace = T,prob = c(0.48,0.52)),
    sample(x = c(0,1),size = 1000,replace = T,prob = c(0.47,0.53))
  ),
  rank = rep(ranks_levels,each=1000),
  pr   = c(
    rep(1,700),rep(0,300), # plat
    rep(1,600),rep(0,400), # dia
    rep(1,550),rep(0,400), # master
    rep(1,500),rep(0,500) # top
  )
) |>
  mutate(rank = factor(rank, levels = ranks_levels,ordered = T))
...
example |>
  mutate(rank = factor(rank, levels = ranks_levels,ordered = F)) |>
  mutate(rank = relevel(rank, ref = "platinum") ) |>
  . => glm(outcome ~ rank, family = "binomial", .) -> model
...
Podcast info: ep.62 ~28:20

> Sometimes the fields is going to influence a deck winrates at different brackets because if the fields is more favorable that contributes to the deck's wr
...
That would be a big error because it assumes there are no other differences in play while the meta by itself, with different meta and so different play rates. Even small difference can bring 





- ZachO explained the concept of Skill-Cap by looking at the performances/win in the ladder by looking at the differences between Top Legends / DiamondI-IV
...
<!-- Sometimes the fields is going to influence a deck winrates at different brackets -->

But after removing all otherr factors and seeing the disparity in MU

But differently for the Skill-Curve they don't look at the WR in function of games but at each MU-WR disparity ( which already big if it goes 2/3% is already considered extremely high skill-cap it is historically high )
...
Assessing the differences in ski
