...
```{r raw-data}
file.Match.DT  <- file.path("C:","LlorR","data","clean","ExampleDT.csv")
LoR.Meta.RMD   <- fread(file.Match.DT, header=T, na.strings = c("",NA))

LoR.Meta.RMD <- LoR.Meta.RMD %>%
  filter("live_2_7" < game_version & game_version < "live_2_9")

nGames <- NROW(LoR.Meta.RMD)
```

```{r get-Matches}
MUtbl <- LoR.Meta.RMD %>%
  filter(game_outcome!="tie") %>%
  select( deck=player,opponent,game_outcome,server,game_version ) %>%
  group_by(deck,opponent) %>%
...
```{r get-WR}
# echo=FALSEMUtbl
WR.DT <- MUtbl[, .(nWIN = sum(muWin),nGames = sum(muGames) ), by=deck ]
...
Sadly, for now, this step it's way simplified: to measure the strength I'm going to use the win rate and to measure the popularity I'll use the play rate. [^1][^2]
...
```{r plot-no-filter, fig.cap="LMI no filter"}
DT.nofil %>%
...
Without filtering we left the outlier cases of *100% WR decks that also have a very small playrate*. With this the "WR index" correspond the raw WR[^3] and just the frequencies are modified.
...
[^1]: If anyone reading this document have a good idea to suggest feel free to suggest it by contacting me.

[^2]: Data from Master rank from Patch 2.7 to 2.8 included.

[^3]: With the max WR = 100% and its complementary to 100 which is 100-100=0, the scale still remain 0 to 100%

## Testing Filters

::: {.panelset}
::: {.panel}
[Filter at 100]{.panel-name}

```{r plot-filter-100, fig.cap="LMI filter at 100 (vS)"}
WR.DT %>%
  filter( nGames > 100 ) %>%
...
::: {.panel}
[Filter at 200]{.panel-name}

```{r plot-filter-200, fig.cap="LMI filter at 200 (vS)"}
WR.DT %>%
  filter( nGames > 200 ) %>%
  mutate( freq_ind = playrate / max(playrate) ) %>%
  mutate( wr_ind   = scale_maxWR(WR) ) %>%
...
::: {.panel}
[Filter at 300]{.panel-name}

```{r plot-filter-300, fig.cap="LMI filter at 300 (vS)"}
WR.DT %>%
  filter( nGames > 300 ) %>%
  mutate( freq_ind = playrate / max(playrate) ) %>%
  mutate( wr_ind   = scale_maxWR(WR) ) %>%
...
```{r plot-noNorm, fig.cap="LMI with no Normalization"}
# DT.noNorm
WR.DT %>%
  filter( nGames > 200 ) %>%
...
```{r compare-LMI-noNorm-vS,fig.cap="Comparing LMI distribution with and without normalizazion (vS methodology)"}
(ggplot(WR.DT %>%
filter( nGames > 200 ) %>%
...
```{r Freq-minmax, fig.cap="Boxplot of Freq Index when applying minmax normalization to: raw values / log-transformation / sqrt"}
...
The log-transformation seems to reduce well the skewness, the question is if it reduce it too much and the sqrt is more appropriate for this case study. I computed LMI for each case (Appendix \@ref(tab:table-minmax)) and looked at the LMI distribution (Fig. \@ref(fig:LMI-minmax))

```{r LMI-minmax, fig.cap="Boxplot of LMI when applying: vS / minmax / log trasnformation + minmax / sqrt + minmax"}
...
```{r plot-minmax-log,fig.cap="LMI / filter at 200 / log-tranformation of playrate / Minmax normalization"}
DT.Norm %>%
  # slice_max(n = 10, order_by = meta_ind, with_ties = F  ) %>%
...
```{r plot-quantile,fig.cap="LMI / filter at 200 / Quantile normalization"}
DT.Quantile %>%
...
```{r plot-aggregation,fig.cap="LMI / filter at 200 / Quantile normalization / Harmonic mean"}
DT.Aggreation %>%
  # slice_max(n = 10, order_by = meta_ind, with_ties = F  ) %>%
  ggplot( aes( x=wr_ind,y=freq_ind )) +
...
```{r plot-Borda,fig.cap="LMI / filter at 200 / Borda method"}
DT.Rank %>%
  # slice_max(n = 10, order_by = meta_ind, with_ties = F  ) %>%
...
```{r ggplotly-LMI,echo=FALSE, layout="l-body-outset"}
...
# <iframe seamless src="../dt1/index.html" width="100%" height="500">
# </iframe>
```
...
rmarkdown::paged_table(WR.DT %>%
  ...
  # slice_head(n = 72) %>%
  select(deck,wr_ind,freq_ind,meta_ind,) %>%
  rename("Deck"="deck","WR index"="wr_ind","Freq ind"="freq_ind","LMI"="meta_ind") %>%
  mutate_if(is.numeric, funs(round(., 2)) )
  ,  options = list(rows.print = 15, cols.print = 5))
...
(\#tab:table-no-filter)Data no Filter games

</caption>

------------------------------------------------------------------------

```{r table-filter-data-200}
rmarkdown::paged_table(DT.filter.vS %>% 
  arrange(desc(meta_ind)) %>% 
  select(deck,wr_ind,freq_ind,meta_ind,) %>%
  rename("Deck"="deck","WR index"="wr_ind","Freq ind"="freq_ind","LMI"="meta_ind") %>%
  mutate_if(is.numeric, funs(round(., 2)) )
...
(\#tab:table-filter-data-200)Data filtered of decks without at least 200 games
...
(\#tab:table-filter-data-200)Data no Normalization
...
(\#tab:table-quantile)Quantile table
...
(\#tab:table-aggregation)Aggregation table
...
```{r LMI-quantile, fig.cap="Boxplot of LMI when applying: vS / quantile / minmax+log trasnformation to playrate / minmax+sqrt transformation to playrate"}
...
```{r}
# htmlwidgets::saveWidget(DT2, file = "dt2.html", selfcontained = TRUE)
# <iframe seamless src="../dt2/index.html" width="100%" height="500">
# </iframe>
