...
What a CI is, is an *extension* of a single point estimation (a single value). By associating a level of confidence, a CI provides a degree of precision of of our estimate, the win-rates.
...
We tried to avoid the use of math for so long, but truly understand some concepts, we need to start using it. Let us start by the very definition of a CI itself.

A confidence interval for a population mean is of the following form $\bar{x} \pm q_{1-\alpha}*SE$
...
ci <- ci_example |>
  rep_sample_n(size = n, reps = n, replace = TRUE) |>
  summarise(
    lower = mean(p) - z_star_95 * (sd(p) / sqrt(n)),
    upper = mean(p) + z_star_95 * (sd(p) / sqrt(n))
  ) |>
...
Each bar is a CI from a sample that either contain (blue) the true win-rate value or it does not contain it (red).

Here we used 100 samples with a confidence level of 95% and as we can notice we were lucky enough to get exactly 95 CI which correctly include the 55% win-rate and 5 who doesn't. As this is dictated by change it the ratio could be different like 93 good CI and 7 bad ones but usually the ratio should be of 95:5.
...
And so we conclude this explanation of confidence intervals. We hope we were able to dissipate the mystery about how they works and also explaining why they are so important.

We hope that other "lor-stats-creators" will one day start using them too. The tool is very very easy to use and should be fairly easy to grasp. Some nuances maybe are not as easy but the very basics should help the most competitive players to better evaluate decks and do better choice.
