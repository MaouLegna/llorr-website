...
Hi, Legna here, normally I try to write those *analysis articles* in a publication-style format, not meant to be read by everyone; this post on the other hand is meant to be an easier read and accessible for most players.
...
Most likely the cadence will be slow. Even this article took weeks not mostly because of a combination of
little time plus needing to decide what and how to write the content.
...
`r kableExtra::text_spec("Example:", color = "blue")` A deck with 52% and
...
So, after this what we must conclude is that one must decide subjectively
...
To better explain what we means we conclude this section with an example taken from a discussion with [Leet](https://twitter.com/leer97):
...
* Vi is a 5-cost single-copy card, should we really assume that the differences in win-rate can be fully explained by that single difference? It is was a full set of lower cost cards the assumption could be worth considering but this case seems to us like pushing the limits.

All this is to say: be careful not to see differences everywhere just because some decks differ by a couple of percentage points. Sometimes it could just be we are focusing too much what nothing seems the equal. At the same time, as mentioned in the before also don't focus too much on the defined rules and be aware of possible differences not yet clearly defined just by a lack of data.
...
it to figures like [Dr.Lor](https://twitter.com/drlor4)
...
What does it actually means? Let's visually recreate the example above
visually.

Assume we have 100 players all equally skilled, all playing the same
deck that we know has a 55% win-rate, also, each player plays 200 games.
...
Each bar is a CI from a player that either contain (blue) the true
win-rate (55%) or it does not contain it (red).

Of the 100 players we can notice we have 6 intervals that doesn't
...
As the black dot represent the win-rate for each player we can notice it
can differ a lot from the expected value. Not only that, sometimes it's
so lower that even as a deck with 55% it would result as a deck with
negative win-rate as it's lower than 50%, in this example
`r neg_winrate` to be exact. This only by the effect of pure chance, no
accounting for skill or MU or anything else.

This is a prime example about why *personal results* are shouldn't be
abused too much and it's usually better to just use aggregate results.
Using personal results could also brings additional issues but that
could be an entire article by itself. For now and this example we hope
it's now clearer when we mention "assuming the CI is one that contains
...
Tab:\@ref(tab:fixed-conf) an example about the effect on the CI range
...
$P( U \leq \theta \leq V)= 1-\alpha$

where U and V are the lower and upper limits of our interval.
...
These equations are hardly clear and for those who did some basic of statistics they should not even seems familiar.

Isn't there a simple expression to define how to the compute the CI of a win-rate?

This is where we use the reality warping power of the Central Limit
Theorem [^4], which, given very easy conditions, it obliterates any
distribution we needs and replace them with a very easy Normal
distribution.
...
Where $z_\alpha$ are the quantile of level of a Normal distribution.

The previous expression can be further rewritten as to obtain the interval defined by

$\mu \pm z_{\alpha/2} \frac{\sigma}{\sqrt(n)}$

and as we are dealing with a proportion
$\hat\mu = p = \frac{\#wins}{\#games}$ and
$\hat\sigma = \hat{p}(1-\hat{p})$ we obtain:

$p \pm z_{1-\alpha/2} \sqrt{\frac{p(1-p)}}{n}}$

After regurgitating all those expressions we are left with a very simple
expression, which is even simpler if we consider that when confidence
level of 95% we have $z_{\alpha/2} = 1.96 \sim 2$

$p \pm 2 \sqrt{\frac{{p(1-p)}}{n}}$

While normally we could stop here we complete this section with a little 'hack'. If we want a very quick computation of the CI and if our win-rate is between 30% and 70%, so pretty much all realistic cases, then $\sqrt{p(1-p)}$ doesn't differ too much from when $p=50\%$ and $p(1-p) = 1/2$ making it so that we could very appropriately obtain

$p \pm \frac{1}{\sqrt{n}}$

Again, this is a very rough approprimation which shouldn't be normally used but it can help to obtain a basic idea about how big our CI should be.
...
The confidence level while impactfull is not usually the one that matter
when creating a confidence level. Even more considering it's usually
fixed at 95% so with associated value of 1.96.
...
the number of games but instead by the root squared number of games.
...
CI's range at fixed confidence level of 95% varying the number of games
...
With the 'hack-approximation' we described above the relationship between the CI's range and the root squared of the number of games should be even easier now.

# Conclusion

And so we conclude this explanation of confidence intervals. We hope we
were able to dissipate the mystery about how they works and also
explaining why they are so important.
...
grasp. Some nuances maybe are not as easy but the very basics should
