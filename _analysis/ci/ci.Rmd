...
**Player B**: "You fool! You clearly didn't see the latest values as mine is obvisously the superior version with 55.5% WR at Master this week!"

**Voice in the background**: "Little did they know both decks were actually the same"

For those who use my content or [Storm](https://twitter.com/storm_lor) [deck tracker](https://app.lormaster.com/meta) one may notice that I use something called Confidence Interval (CI) or similarly Storm add a $\pm X$ value to a deck win-rate. That is a confidence interval.

While the intuition may easily tell how one is supposed to read them, it's really, really easy to misunderstand them. Here, We'll explain what they are, how they are computed, why you should almost always require their use and also why/how you shouldn't look too much into them.

If the last two points seems a contradiction they are not but we will try to convey my reasoning.
...
>
> I would like to say that this is the first of a series of articles meant to explain some statistical concepts in order to better understand the data and their nuances but I can hardly guarantee it, I will try, but most likely the cadence will be slow.

Let us start with two examples:

* Between a deck with a 55% WR made of 20 games and a deck with 53% WR made of 100.000 games which deck should one normally choose?

> The second one

* Suppose it wasn't possible to collect all games at Master, you have to rely on a sample of games. On Week-1 you have deck-A with a 10.2% play-rate and 52% win-rate, on Week-2 it reached 10.4% play-rate and 52.2% win-rate. Clearly the deck is performing better in Week-2 compared to Week-1

> Most likely it is not

In the first example, if ones go by typical level of a twitter analysis 55% > 53% and the first deck is the better choice. Most most people will disagree as 53% is the more stable value. Why is it that?

Because most people knows that the more examples you have the more the data will converge to its *true value* ^[which still requires an infinite sample] and intuitively they applied some statistical inference to the data, either with the use of confidence intervals or statistical testing ( they are related after all )

The example in this case was too easy, so it was *easy* to answer. Now, let's say a deck-A has a 55% win-rate with 150 games and deck-B has a 56% win-rate with 100 games. Is it still that easy to discern the difference just by the numbers of games? Of course not, and here is where the confidence interval would help us.

The second one is not as clear, but the point we wanted to convey is that with some differences, their values are not statistically different from zero and it would be better to just ignore them.

To be fair, as some data like the Master population games can be considered a full collection of their games and not a sample, we are not dealing with incomplete information and estimates but their actual values, so each difference found is indeed something that exist. Still the concept that some differences are better off to be ignored remains true.

Again, this consideration is also a result of using confidence intervals and/or statistical testing.

# Reject Points, embrace Intervals
