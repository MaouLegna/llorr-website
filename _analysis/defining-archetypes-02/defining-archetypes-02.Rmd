...
```{r Rubin-fix}
LoR.Deck |>
  filter(factions =="faction_MtTargon_Name,faction_ShadowIsles_Name") |>
  rowwise() |>
  mutate(key_search = sum(str_detect(c_across(contains("Card")), getCardCode("Mistwraith")),na.rm = T)) |>
  filter(key_search==3) |>
  mutate(key_search = sum(str_detect(c_across(contains("Card")), getCardCode("Pale Cascade")),na.rm = T)) |>
  filter(key_search==3) |>
  pull(deck_code)
```
