...
# 
# apres1a <- apcluster(negDistMat(r=2), iris)
# 
# 
...
# top_n_codes(DT = patch214215code,
#             archetypes = archetypes1,
#             n = 10,dopull = F)

#' codes
exCodes1 <- top_n_codes(DT = patch214215code,archetypes = archetypes1,n = 10,dopull = T)
exCodes2 <- top_n_codes(DT = patch214215code,archetypes = archetypes2,n = 10,dopull = T)
exCodes3 <- top_n_codes(DT = patch214215code,archetypes = archetypes3,n = 10,dopull = T)

# set.seed(123)
# exCodes1 <- LoR.Deck.RMD[ archetype %in% archetypes1, sample(deck_code,size = 20), by=archetype ]
# exCodes2 <- LoR.Deck.RMD[ archetype %in% archetypes2, sample(deck_code,size = 20), by=archetype ]
# exCodes3 <- LoR.Deck.RMD[ archetype %in% archetypes3, sample(deck_code,size = 20), by=archetype ]
# 
# #' distance matrix
DSim_ex1 <- cos.dsimMatrix(exCodes1)
DSim_ex2 <- cos.dsimMatrix(exCodes2)
DSim_ex3 <- cos.dsimMatrix(exCodes3)
...
set.seed(123)
ex1.hc.complete.n <- hclust(DSim_ex1, method = "complete", members = sample(1:5000,60))

ex1.hc.single   <- hclust(DSim_ex1, method = "single")

ex1.hc.average  <- hclust(DSim_ex1, method = "average")
set.seed(123)
ex1.hc.average.n  <- hclust(DSim_ex1, method = "average", members = sample(1:5000,60))

ex1.hc.centroid <- hclust(DSim_ex1, method = "centroid")
set.seed(123)
ex1.hc.centroid.n <- hclust(DSim_ex1, method = "centroid", members = sample(1:5000,60))
