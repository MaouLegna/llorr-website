...
```{r}
ExampleDT1 <- LoR.Melt.Matches.RMD |>
  # tabyl(player) |>
  # arrange(desc(n))
  filter( player %in% c( "Draven / Sion (NX/PZ)","Nami / Zoe","Senna / Veigar (BC/SI)","Gangplank / Twisted Fate (BC/BW)","Poppy / Ziggs (BC/NX)" ) ) |>
  group_by(player) |>
  count(deck_code) |>
  # arrange(player,desc(n)) |>
  top_n(5)
```
```{r}
ExampleDT2 <- LoR.Melt.Matches.RMD |>
  filter( str_detect(factions,"Noxus") ) |>
  # tabyl(player) |>
  # arrange(desc(n))
  filter( player %in% c( "Draven / Sion (NX/PZ)","Poppy / Ziggs (BC/NX)","Caitlyn / Draven","Gangplank / Miss Fortune (BW/NX)","Draven / Jinx" ) ) |>
  group_by(player) |>
  count(deck_code) |>
  # arrange(player,desc(n)) |>
  top_n(5)
...
```{r}
ExampleDT3 <- LoR.Melt.Matches.RMD |>
  filter( str_detect(factions,"Noxus") ) |>
  # tabyl(player) |>
  # arrange(desc(n))
  filter( player %in% c( "Draven / Sion (NX/PZ)","RubinBait - Draven / Sion","RubinBait - Draven / Ezreal","RubinBait - BandleBurn" ) ) |>
  group_by(player) |>
  count(deck_code) |>
  # arrange(player,desc(n)) |>
  top_n(5)

codes <- ExampleDT3$deck_code
```
...
DSim_ex1 <- eisen_cos.sim(deck.matrix)
