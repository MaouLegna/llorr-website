...
From this an algorithm would be equivalent to start in the middle of its steps in the middle of the cluster definition and so not starting to work with the leaf but clusters with different masses.
...
2.  Should there exist outliers that's best no to assign to a cluster?

Many cluster algorithm are sensitive to noise and outliers.
AHC for example is able to identify smaller clusters.
If an outlier is wrongly assigned to a cluster in some cases this error will be propageted on all the following steps with no way to intervene.
