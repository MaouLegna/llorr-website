...
To give a reference the Between Worlds Season from patch 2.18 to 2.20 featured at Master rank ~`r floor(NROW(S10Decks)/1000)*1000`
...
**This makes the choice and/or application of certain method/algorithms and so on are not feasible at high dimension.**
...
```{r}
format(object.size(cos.dsim.arch3.20), units = "Mb")
```

While the size for a case with 100 decks, a 5500x5500 decks is:

```{r}
format(object.size(cos.dsim.arch3.100), units = "Mb")
```
Quite the increase while still far from the number of decks of a patch, still an order of magnitude lower than that. And yet using a patch data is already a simplification for the overall problem.

At the moment of the writing of this article our Deck's dataset is in fact made of 1M decks.
