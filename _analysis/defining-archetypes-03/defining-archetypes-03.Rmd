...
LoR.Match.RMD.old  <- fread(file.path("C:","LlorR","data","raw","LoR_MatchDT_patch_2_04_to_17final.csv"), header = T, na.strings = c("", NA))
...
DeckArchetype3 <- fread("./data/example_archetye_03.csv") |>
  select(deck_code,factions) |>
  mutate(
    
    # faction - clean the names
    factions = str_remove_all(factions, pattern = paste(c("_Name","faction_"), collapse = "|")),
    # faction - turn them into the abbreviation
    factions = str_replace_all(factions, set_names(data_regions$abbreviation, data_regions$nameRef)),
    
    # card - extract the card list # deprecated
    # cards_list = map(.x = deck_code, .f = ~lordecks::get_decklist_from_code(.x,format = "simple") ),
    
    # card - extract the card list with each card separate
    cards_list = map(.x = deck_code, .f = ~get_deck_from_code(.x)),
    # card - turn the card list into a single string
    cards = map_chr(cards_list, str_flatten, collapse = " "),
    # extract CARDS region
    cards.region = map(.x = cards_list, ~str_remove_all(.x, pattern = "[0-9]")),
    
    # champs - extract the champions cardCodes
    champs = str_extract_all(cards, pattern = paste(LoR.Champion$cardCode, collapse = "|")),
    
    # region - extract CHAMPION region
    champs.region = map(.x = champs, ~sort(unique(str_remove_all(.x, pattern = "[0-9]")))),
    champs.region = map(champs.region,    ~sort(factor(.x,ordered = T,levels = data_regions$abbreviation))),
    champs.region = map_chr(champs.region, str_flatten, collapse = ","),
    
    # allegiance - turn them into the name value
    allegiance = str_extract_all(cards, pattern = paste(LoR.Allegiance$cardCode, collapse = "|")),
    
    # nRegion (table) ----
    nRegion = map(.x = cards.region, ~tabyl(.x)[,1:2]|>rename_all(~c("region","region.n")) ), # percent is not necessary
    
    # reduce the card.region after counting the cards
    cards.region = map(.x = cards.region, ~unique(.x)),
    
    
    # champion - sort them
    champs = map(.x = champs, ~unique(.x) ),
    # champion - turn them into the name value
    champs = map(.x = champs, ~str_replace_all(.x, set_names(LoR.Champion$name, LoR.Champion$cardCode))),
    # champion - sort them
    champs = map(.x = champs, ~sort(.x) ),
    # champion - flatten the vector
    champs = map_chr(champs, str_flatten, collapse = "/"),
    # champion - fix no champs
    # champs = ifelse(grepl("^( )", x), paste0("No Champions", x), x))
    champs = replace(champs,champs=="","Championless"),
    
    # when dealing with dual regions
    cards.fix = map2(cards_list,factions,~fix_deck_codes(.x,.y)),
    cards.region.fix = map(.x = cards.fix,        ~str_remove_all(.x, pattern = "[0-9]")),
    nRegion.fix      = map(.x = cards.region.fix, ~tabyl(.x)[,1:2]|>rename_all(~c("region.fix","region.n.fix")) ), # percent is not necessary
    cards.region.fix = map(.x = cards.region.fix, ~unique(.x)),
    
    # fix - do I have to add the region tag?
    # no_fix = map2_lgl(.x = factions, .y = region, .f = ~identical(.x,.y ))
    # factions = str_replace_all(factions, ",", "/"),
    # archetype ----
    
    cards.region     = map(cards.region,    ~sort(factor(.x,ordered = T,levels = data_regions$abbreviation))),
    cards.region.fix = map(cards.region.fix,~sort(factor(.x,ordered = T,levels = data_regions$abbreviation))),
    
    archetype = pmap(.l = list(champs,factions,champs.region), ~ifelse( !identical(..2,..3) , glue("{..1} ({..2})"), ..1  )),
    archetype = str_replace_all(archetype, pattern = ",", replacement = "/"),
