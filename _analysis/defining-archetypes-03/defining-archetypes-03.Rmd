...
fread(file.path("C:","LlorR","data","raw","old-data","LoR_MatchDT_S10_218_220.csv"), select=c("game_type","deck_code_1","deck_code_2"), header = T, na.strings = c("", NA)) |>
  filter(game_type=="Ranked") |>
  select(-game_type) |>
  pivot_longer(cols = contains("deck_code"),values_to = "deck_code",names_to = NULL) |>
  distinct(deck_code) -> S10Decks
```
Deck-list from "Master" games during the Between Worlds Season - patch [2.18,2.21) ~`r floor(NROW(S10Decks)/1000)*1000`
...
```{r allegiance-mess}
S10Decks[ !is.na(cards.region.fix) , cards.region:=cards.region.fix  ]
...
str_split(exampleAlle$allegiance,pattern = ",")
...
S10Decks |>
  ...
  slice_head(n= 10) |>
  mutate(
    cards_list_try = map(.x = deck_code, .f = ~get_deck_from_code(.x))
  )
