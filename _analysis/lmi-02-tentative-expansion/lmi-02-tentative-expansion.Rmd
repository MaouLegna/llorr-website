...
This can be done when comparing the result of [Balco's Meta-Score](https://lor-meta.com/tierlist/) which applies the same algorithm by vS but is not part of this article as we would also include the opinion of third-party experts (some high level players) to evaluate the different CI performances.
...
fig <- LMI.DT %>%
  ...
  rename("Tier"="tier","Deck"="deck","Win_Rate"="WR","Play_Rate"="playrate","Ban Rate"="meanBan","LMI"="LMI") %>%
  plot_ly(
    type = 'scatter',
    mode = 'markers',
    x = ~`str_dim`,
    ...
    ) %>% layout(xaxis = list(title = "Strenght Index",titlefont = f),
                 yaxis = list(title = "Popularity Index",titlefont = f),
