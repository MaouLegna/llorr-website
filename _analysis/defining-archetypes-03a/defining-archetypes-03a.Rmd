...
The main issue to use 'real-case' relates to the amount of decks to use and the resulting computational load.

Distance Matrices and consequently all algorithm derived by them have a quadratic growth ($O(n^2)$) so scalability is a problem.

This project to "define archetypes" is not meant to be purely theoretical, the aim is to also apply it to the collected data and provide the results, be it in the reports or any other content.

Yet, the problem not feasible at high dimension even assuming the analysis is self contained and we have more time to apply any algorithm.
...
A more appropriate time-frame can be a single competitive Season; we will create a dataset created to that is has a similar scale of the S10-MasterRank (42000) decks mentioned before.
...
The required distance matrix would be too big (44000^2) and we wouldn't even be able to allocate enough memory to work with it.
...
We now wants to highlight that it would be almost a waste of resources.
