...
To give a reference of a 'real-case' number, the Between Worlds (S10) Season which lasted from patch 2.18 to 2.20 featured at Master rank ~`r floor(NROW(S10Decks)/1000)*1000` decks.
...
What is the main issue with data from 'real-case'? It mainly relates to two factor: the *dirtiness* of the data but mostly the amount of decks to use and the resulting computational load.

Distance Matrices and consequently all algorithm derived by them have a quadratic growth ($O(n^2)$) so scalability is a problem as this project to "define archetypes" is not meant to be purely theoretical. The aim of this project is to also apply it to the collected data and provide the results, be it in the reports or any other content.

Yet, the problem is not feasible at high dimension even assuming the analysis is self contained and we have an infinite amount of time to apply any algorithm.

One may not be used to these context so let us give some numbers of the problem at hand:

At the moment of the writing of this article our Deck's dataset is made of neatly 1M decks. The dataset contains only 40-cards decks which has been played in Constructed or PvP modes. [^1]
...
As we are not ready yet to just simpoly apply any methods to any kind of decks data we will create a dataset so that is has a similar scale of the S10-MasterRank (42000) decks mentioned before.
...
Now, let us explain why it would be impossible to even try to apply the algorithm/code of the previous article.
...
* By using all the 44k decks we can't even try to recreate the distance matrix, it would require quite the machine and we can infer that just that matrix would be of around  `r 116*(44000^2/5500^2)` Mb. The required distance matrix would be too big (44000^2) and we wouldn't even be able to allocate enough memory to work with it.
...
Let us assume that we are able to solve the computation and allocation of the distance matrix.

This still would not be enough, because, every aspect that uses it will still be affected by the same computational burden.

We now wants to explain how it would be 'a waste' of resources.

To explain this we will show the resulting heatmap of a couple of subsets of the example dataset. One with a subset of 1100 not completely at random decks and another one with 5500 decks.
...
The resulting heatamap with 5500 decks heatamap is:
...
As we can see in Fig:\@ref(fig:Heat1100) and Fig:\@ref(fig:Heat5500) there are clear indication of the presence of several small clusters (the yellow areas) but the majority, the vast majority of the matrix the distance is at its max or we can the similarity is null.

While it's true that this result is like this partially because of how the example matrix was created, even assuming actual "real-data" the plot wouldn't differ by much.

The main reason is to simply because by the limitation imposed by the deck building just can't allow to have huge clusters compared to the one if the diagonal from our example. There could be exceptions but for the most part it would imply that certain regions and cards would vastly dominate their playrate across a huge amount of decks.
